...
from scara_2dof_robot_env import Scara_2Dof_Robot_Env
...
def main():
    ...
    arm_length_limits = np.array([1.0, 5.0])
    ...
    batch_size_train = n_steps_train
    global_iteration = 0
    TRAIN = False
    ...
    while True:
        ...
        env_fns = [lambda config=config: Scara_2Dof_Robot_Env(**config) for config in env_configs]

        # Create the vectorized environment using SubprocVecEnv directly
        vec_env = SubprocVecEnv(env_fns, start_method='fork')
        model_name = f"ScaraRobot__length_{CALL_BACK_FUNC}_batchsize{batch_size_train}_start_upd_{n_update_dist}lr_mean{lr_std_schaff}_std_{lr_std_schaff}_sde{use_sde}_Tanh_Tsteps_{total_timesteps_train}_lr_{learning_rate_train}_hidden_sizes_{hidden_sizes_train}_POSreward_{REWARD[0]}_VELreward_{REWARD[1]}_omega_pen_{REWARD[2]}_var_design"
        log_dir = f"{REPO_ROOT}/python/scara_2dof_robot/PlanarRobot_tensorboard/TB_{model_name}"

        if LOAD_OLD_MODEL is True:
            new_model = []
            old_model = PPO.load(f"{REPO_ROOT}/python/scara_2dof_robot/rl/trained_model/random_design/ScaraRobot_FilterOn_length_random_design_start_upd_25lr_mean0.01_std_0.01_sdeFalse_Tanh_Tsteps_409600000_lr_0.001_hidden_sizes_50_POSreward_1.0_VELreward_0.0_omega_pen_0.0_var_design", env = vec_env)
        ...
        new_model.save(f"{REPO_ROOT}/python/scara_2dof_robot/rl/trained_model/random_design/{model_name}")
        print("Model saved")
        LOAD_OLD_MODEL = True
        vec_env.close()
        break
...
class constant_design(BaseCallback):
    ...
    def _on_rollout_end(self) -> bool:
        self.model.save(f"{REPO_ROOT}/python/scara_2dof_robot/rl/trained_model/random_design/{self.model_name}")
    ...
    def _on_step(self) -> bool:
        ...
        if self.num_timesteps > self.steps_update_distribution:
            ...
            if self.num_timesteps % self.save_recorded_data == 0:
                ...
                self.mat_iter += 1
                file_path = f"{REPO_ROOT}/outputs/scara_2dof_robot/{self.mat_file_name}_{self.mat_iter}.mat"
                savemat(file_path, output_data)
                self.model.save(f"{REPO_ROOT}/python/scara_2dof_robot/rl/trained_model/{self.model_name}_{self.mat_iter}")
...
class Hebo_callback(BaseCallback):
    ...
    def on_training_start(self, locals_: Dict[str, Any], globals_: Dict[str, Any]) -> bool:
        ...
        for i in range(self.batch_size_opt):
    ...
    def _on_rollout_start(self) -> bool:
        ...
        for i in range(self.n_envs_train//self.batch_size_opt):
            for j in range(self.batch_size_opt):
                ...
                self.training_env.env_method('set_target_pos_tcp', self.target_pos_param[j][0],self.target_pos_param[j][1], indices=[i*self.batch_size_opt + j])
                self.training_env.env_method('set_init_joint_pos', self.init_joint_pos[j], indices=[i*self.batch_size_opt + j])
                self.training_env.env_method('set_env_id', i, indices=[i*self.batch_size_opt + j])
                ...
                current_arm_length = self.training_env.env_method('get_arm_length', indices=[i*self.batch_size_opt + j])[0]
                target_pos_tcp = self.training_env.env_method('get_target_pos_tcp', indices=[i*self.batch_size_opt + j])[0]
                init_joint_pos = self.training_env.env_method('get_curr_joint_pos', indices=[i*self.batch_size_opt + j])[0]
                print(f"env: {i*self.batch_size_opt + j:<1.2f}, real id:{dist_env_id}, arm length: {new_arm_length}, curent_arm_length: {current_arm_length} , target pos: {target_pos_tcp}, init joint pos: {init_joint_pos}")
