...
from scara_2dof_robot_env import Scara_2Dof_Robot_Env  # Import your custom environment
from stable_baselines3.common.policies import ActorCriticPolicy
# Define your callback classes here
...
# Define your callback classes here (e.g., Hebo_callback, Schaff_callback, random_design, constant_design)

def main():
    # Training parameters
    hidden_sizes_train = 256
    REWARD = np.array([1.0, 0.0, 0.1])
    learning_rate_train = 0.0001
    LOAD_OLD_MODEL = False
    n_steps_train = 512 * 5
    n_envs_train = 100
    total_timesteps_train = n_steps_train * n_envs_train * 1000
    model_name = "PlanarRobot_SAC_Model"
    log_dir = f"{REPO_ROOT}/python/scara_2dof_robot/PlanarRobot_tensorboard/TB_SAC"
    CALL_BACK_FUNC = "constant_design" # "Hebo_callback", "Schaff_callback", "random_design", "constant_design"

    # Define unique initialization variables for each environment
    env_configs = [{'REWARD': REWARD, 'env_id': i, 'call_back' : "random_design"} for i in range(n_envs_train)]
    ...
    policy_kwargs = dict(activation_fn=torch.nn.Tanh, net_arch=[hidden_sizes_train, hidden_sizes_train])

    model = SAC("MlpPolicy", env=vec_env, learning_rate=learning_rate_train,
                tensorboard_log=log_dir, batch_size=n_steps_train, train_freq = n_envs_train,
                device ='cpu', policy_kwargs=policy_kwargs)

    # Callback function selection
    if CALL_BACK_FUNC == "Hebo_callback":
        callback = Hebo_callback(...)
    elif CALL_BACK_FUNC == "Schaff_callback":
        callback = Schaff_callback(...)
    elif CALL_BACK_FUNC == "random_design":
        callback = random_design(...)
    elif CALL_BACK_FUNC == "constant_design":
        callback = constant_design(...)
    else:
        print("No callback function specified")
        return

    # Train the model
    model.learn(total_timesteps=total_timesteps_train, progress_bar=True, callback=callback)

    # Save the trained model
    model.save(f"{REPO_ROOT}/python/scara_2dof_robot/rl/trained_model/SAC/{model_name}")
...
class Hebo_callback(BaseCallback):
    ...
    def _on_rollout_end(self) -> bool:
        ...
        if self.num_timesteps % self.save_recorded_data == 0:
            ...
            file_path = f"{REPO_ROOT}/outputs/scara_2dof_robot/{self.mat_file_name}.mat"
            savemat(file_path, output_data)
            print("saving current model...")
            self.model.save(f"{REPO_ROOT}/python/scara_2dof_robot/rl/trained_model/{self.model_name}")
            print("Model saved")
