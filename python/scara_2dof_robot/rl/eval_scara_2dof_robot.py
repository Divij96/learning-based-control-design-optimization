...
def create_target_marker(target_pos, diameter, frame_id="map", color=np.array([1, 0, 0]) ,marker_name=f"target_pos"):
    """Create a Marker message to represent the robot arm."""
    marker = Marker()
    marker.header.frame_id = frame_id
...
def main():
    ...
    arm_lengths = [1.25, 1.25]  # Lengths of the arms
    diameters = [0.1, 0.1]  # Diameter of the arms
    eval_env = Scara_2Dof_Robot_Env(env_id = 1)
    ...
    EVAL_MODEL_PATH = f"{REPO_ROOT}/python/scara_2dof_robot/rl/trained_model/random_design/ScaraRobot_FilterOn_length_random_design_start_upd_25lr_mean0.01_std_0.01_sdeFalse_Tanh_Tsteps_409600000_lr_0.001_hidden_sizes_50_POSreward_1.0_VELreward_0.0_omega_pen_0.0_var_design"
    ...
    while not rospy.is_shutdown():
        ...
        target_marker = create_target_marker(target_pos, 0.2, color=np.array([1, 0, 0]))
        target_tcp_pos = info['pos_tcp']
        target_tcp_marker = create_target_marker(target_tcp_pos, 0.2, color=np.array([0, 1, 0]),marker_name=f"target_pos_tcp")
