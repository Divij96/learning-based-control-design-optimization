...
def main():
    ...
    while True:
        ...
        log_dir = f"{REPO_ROOT}/python/quadcopter/Quadcopter_tensorboard/TB_{model_name}"

        if LOAD_OLD_MODEL is True:
            new_model = []
            old_model = PPO.load(f"{REPO_ROOT}/python/quadcopter/rl/trained_model/random_design/QuadcopterCircle_all_heboBatch_10_envs_train_20_batch_size_train_256_random_design_start_upd_25lr_mean0.01_std_0.01_sdeFalse_Tanh_Tsteps_4096000_lr_0.0001_hidden_sizes_256_POSreward_0.0_VELreward_1.0_omega_pen_0.2_var_design", env = vec_env)
        ...
        new_model.save(f"{REPO_ROOT}/python/quadcopter/rl/trained_model/random_design/{model_name}")
        print("Model saved")
        LOAD_OLD_MODEL = True
        vec_env.close()
        break
...
class random_design(BaseCallback):
    ...
    def _on_rollout_end(self) -> bool:
        ...
        self.average_reward = []
        self.design_iteration = [0 for _ in range(self.n_envs_train)]
        self.model.save(f"{REPO_ROOT}/python/quadcopter/rl/trained_model/random_design/{self.model_name}")
        return True
    ...
    def _on_step(self) -> bool:
        ...
        if self.num_timesteps > self.steps_update_distribution:
            ...
            if self.num_timesteps % self.save_recorded_data == 0:
                ...
                self.mat_iter += 1
                file_path = f"{REPO_ROOT}/outputs/quadcopter/{self.mat_file_name}_{self.mat_iter}.mat"
                savemat(file_path, output_data)
                self.model.save(f"{REPO_ROOT}/python/quadcopter/rl/trained_model/{self.model_name}_{self.mat_iter}")
...
class Hebo_callback(BaseCallback):
    ...
    def _on_rollout_end(self) -> bool:
        ...
        if self.num_timesteps % self.save_recorded_data == 0:
            ...
            file_path = f"{REPO_ROOT}/outputs/quadcopter/{self.mat_file_name}.mat"
            savemat(file_path, output_data)
