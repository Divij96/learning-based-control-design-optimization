...
class CustomNetwork(nn.Module):
    ...
    def __init__(self, observation_space, net_arch, activation_fn, design_obs=2, trajectory_obs=2):
...
def main():
    ...
    arm_length_limits = np.array([0.1, 5.0])
    ...
    while True:
        ...
        env_configs = [{'REWARD': REWARD, 'env_id': i, 'call_back' : CALL_BACK_FUNC} for i in range(n_envs_train)]
        ...
        env_fns = [lambda config=config: Robot_6dof_Arm_Dynamics(**config) for config in env_configs]

        # Create the vectorized environment using SubprocVecEnv directly
        vec_env = SubprocVecEnv(env_fns, start_method='fork')
        model_name = f"ScaraRobot_dynamics_6arm_{CALL_BACK_FUNC}_start_upd_{n_update_dist}lr_mean{lr_std_schaff}_std_{lr_std_schaff}_sde{use_sde}_Tanh_Tsteps_{total_timesteps_train}_lr_{learning_rate_train}_hidden_sizes_{hidden_sizes_train}_POSreward_{REWARD[0]}_VELreward_{REWARD[1]}_omega_pen_{REWARD[2]}_var_design"
        log_dir = f"{REPO_ROOT}/python/forward_dynamics_robot_arm/PlanarRobot_tensorboard/TB_{model_name}"

        if LOAD_OLD_MODEL is True:
            new_model = []
            old_model = PPO.load(f"{REPO_ROOT}/python/forward_dynamics_robot_arm/rl/trained_model/random_design/ScaraRobot_dynamics_random_design_start_upd_25lr_mean0.01_std_0.01_sdeFalse_Tanh_Tsteps_20480000_lr_0.0001_hidden_sizes_256_POSreward_1.0_VELreward_0.0_omega_pen_0.0_var_design", env = vec_env)
        ...
        if CALL_BACK_FUNC == "Hebo_callback":
            ...
            param_changer = constant_design(model_name = model_name, n_steps_train = n_steps_train, n_envs_train = n_envs_train, verbose=1)
        ...
        new_model.save(f"{REPO_ROOT}/python/forward_dynamics_robot_arm/rl/trained_model/random_design/{model_name}")
        print("Model saved")
        LOAD_OLD_MODEL = True
        vec_env.close()
        break
...
class constant_design(BaseCallback):
            ...
            if 'dones' in self.locals:
                ...
                for i, done in enumerate(dones):
                    if done or self.episode_length[i] >= self.n_steps_train:
                        ...
                        print(f"Env ID: {dist_env_id}, episode reward: {self.episode_rewards[i]},  Mean episode length: {self.episode_length[i]}, arm length: {current_arm_length}, target pos: {target_pos_tcp}")
    ...
    def _on_rollout_end(self) -> bool:
        self.model.save(f"{REPO_ROOT}/python/forward_dynamics_robot_arm/rl/trained_model/random_design/{self.model_name}")
        self.logger.record("mean episode length", np.sum(self.average_episode_length) / self.n_envs_train)
        self.logger.record("mean reward", np.sum(self.average_reward) / self.n_envs_train)
        self.average_episode_length = []
        self.average_reward = []
        return True
    ...
    def _on_step(self) -> bool:
        ...
        if 'rewards' in self.locals:
            ...
            for i, reward in enumerate(rewards):
                self.episode_rewards[i] = self.episode_rewards.get(i, 0) + reward
                self.episode_length[i] = self.episode_length.get(i, 0) + 1
                con = self.training_env.env_method('get_continue_training', indices=[0])[0]
    ...
    def _on_step(self) -> bool:
        ...
        if self.num_timesteps > self.steps_update_distribution:
            ...
            if self.num_timesteps % self.save_recorded_data == 0:
                ...
                self.mat_iter += 1
                file_path = f"{REPO_ROOT}/outputs/forward_dynamics_robot_arm/{self.mat_file_name}_{self.mat_iter}.mat"
                savemat(file_path, output_data)
                self.model.save(f"{REPO_ROOT}/python/forward_dynamics_robot_arm/rl/trained_model/{self.model_name}_{self.mat_iter}")
...
class Hebo_callback(BaseCallback):
    ...
    def __init__(self,model_name = f"matfile" ,n_steps_train=512 * 10, n_envs_train=8, arm_length_limits = np.array([1.0, 5.0]), verbose=0):
    ...
    def on_training_start(self, locals_: Dict[str, Any], globals_: Dict[str, Any]) -> bool:
        ...
        #generate random target positions equal to batch size
        for i in range(self.batch_size_hebo):
            ...
            self.target_pos_param.append([theta, radius])
            self.init_joint_pos.append(np.array([random.uniform(self.min_joint_pos[0], self.max_joint_pos[0]),
    ...
    def _on_rollout_start(self) -> bool:
        ...
        for i in range(self.n_envs_train//self.batch_size_hebo):
            for j in range(self.batch_size_hebo):
                ...
                self.training_env.env_method('set_arm_length', new_arm_length, indices=[i*self.batch_size_hebo + j])
                self.training_env.env_method('set_target_pos_tcp', self.target_pos_param[j][0],self.target_pos_param[j][1], indices=[i*self.batch_size_hebo + j])
                self.training_env.env_method('set_init_joint_pos', self.init_joint_pos[j], indices=[i*self.batch_size_hebo + j])
                self.training_env.env_method('set_env_id', i, indices=[i*self.batch_size_hebo + j])
                ...
                current_arm_length = self.training_env.env_method('get_arm_length', indices=[i*self.batch_size_hebo + j])[0]
                target_pos_tcp = self.training_env.env_method('get_target_pos_tcp', indices=[i*self.batch_size_hebo + j])[0]
                init_joint_pos = self.training_env.env_method('get_curr_joint_pos', indices=[i*self.batch_size_hebo + j])[0]
                print(f"env: {i*self.batch_size_hebo + j:<1.2f}, real id:{dist_env_id}, arm length: {new_arm_length}, curent_arm_length: {current_arm_length} , target pos: {target_pos_tcp}, init joint pos: {init_joint_pos}")
    ...
    def _on_rollout_end(self) -> bool:
        ...
        if self.num_timesteps >= self.steps_update_distribution-self.n_envs_train:
            ...
            for i in range(self.n_envs_train//self.batch_size_hebo):
                ...
                # Logging
                current_arm_length = self.training_env.env_method('get_arm_length', indices=[i*self.batch_size_hebo])[0]
        ...
        if self.num_timesteps % self.save_recorded_data == 0:
            ...
            file_path = f"{REPO_ROOT}/outputs/robots/{self.mat_file_name}.mat"
            savemat(file_path, output_data)

        #increasing the batch size with each iteration
        # if self.num_timesteps % self.reduce_batch_size == 0:
        #     print("Increasing batch size...")
        #     self.batch_size_hebo = 2 * self.batch_size_hebo
        #     if self.batch_size_hebo > self.n_envs_train:
        #         self.batch_size_hebo = self.n_envs_train
    ...
    def _on_step(self) -> bool:
        ...
        if self.num_timesteps >= self.steps_update_distribution-self.n_envs_train:
            ...
            if 'dones' in self.locals:
                dones = self.locals['dones']
                for i, done in enumerate(dones):
                    if done or self.episode_length[i] >= self.batch_iterations:
                        self.design_iteration[i] += 1
        return True

class Hebo_Gauss_callback(BaseCallback):

    def __init__(self,model_name = f"matfile", model = None ,n_steps_train=512 * 10, n_envs_train=8, arm_length_limits = np.array([1.0, 5.0]), verbose=0):
        ...
        self.n_envs_train = n_envs_train
        self.model_name = model_name
        self.min_bound = arm_length_limits[0]
        self.max_bound = arm_length_limits[1]
        self.model = model
        self.distributions = []
        self.mat_arm_length = []
        self.mat_reward = []
        ...
        self.design_space_lb = np.array([0.1, 0.1, 0.1])
        self.design_space_ub = np.array([2.0, 2.0, 2.0])
        self.design_space_history = []
        self.best_design_gauss = np.array([0.1, 0.1, 0.1])
        self.best_design_reward_gauss = -1000
        # Range of possible components
        self.n_components_range = range(1, 6)
    ...
    def _on_rollout_start(self) -> bool:
        ...
        if self.state == "hebo_init":
            ...
            for i in range(1, len(self.design_space_lb) + 1):
                space_config.append({
                    ...
                    'type': 'num',
                    'lb': self.design_space_lb[i-1] * 100,  # Assuming self.min_arm_length is defined
                    'ub': self.design_space_ub[i-1] * 100   # Assuming self.max_arm_length is defined
                })
        ...
        if self.state == "gauss_init":
            ...
            top_indices = [i for i in top_indices if self.hebo_reward_history[i] > avg_reward]
            print(f"avg reward: {avg_reward}, top indices: {len(top_indices)}")

            top_designs = self.hebo_design_history[top_indices] / 100
            top_rewards = self.hebo_reward_history[top_indices]
            #find the lower and upper bound of the top designs for each dimension

            self.design_space_lb = np.min(top_designs, axis=0)
            self.design_space_ub = np.max(top_designs, axis=0)
            ...
            #scale top_designs to the design space bounded by lb and ub
            top_designs_scaled = (top_designs - self.design_space_lb)/(self.design_space_ub - self.design_space_lb)
            ...
            self.optimal_gmm.fit(top_designs_scaled)

            print(f"GMM means: {self.optimal_gmm.means_}")
            print(f"GMM covariances: {self.optimal_gmm.covariances_}")
            print(f"GMM means scaled: {self.optimal_gmm.means_ * (self.design_space_ub - self.design_space_lb) + self.design_space_lb}")
            print(f"GMM covariances scaled: {self.optimal_gmm.covariances_ * (self.design_space_ub - self.design_space_lb)}")
            self.hebo_design_history = []
            self.hebo_reward_history = []
        ...
        if self.state == "hebo":
            ...
            if not self.sample_half_gauss:
                ...
                print(f"gauss_output: {gauss_output_scaled[0]}")
                ...
                # Assuming the column names of self.rec are 'x1', 'x2', and 'x3'
                column_names = ['x1', 'x2', 'x3']

                # Convert gauss_output[0] to a DataFrame with the same column names
                gauss_output_df = pd.DataFrame(gauss_output*100, columns=column_names)

                # Now concatenate the DataFrames
                self.rec = pd.concat([self.rec, gauss_output_df], ignore_index=True)
            ...
            for i in range(self.n_envs_train//self.batch_size_hebo):
                for j in range(self.batch_size_hebo):
                    ...
                    self.training_env.env_method('set_arm_length', new_arm_length/100, indices=[i*self.batch_size_hebo + j])
                    self.training_env.env_method('set_env_id', i, indices=[i*self.batch_size_hebo + j])
        ...
        if self.state == "gauss":
            ...
            for i in range(self.n_envs_train//self.batch_size_gauss):
                for j in range(self.batch_size_gauss):
                    self.training_env.env_method('set_arm_length', self.gauss_designs[i], indices=[i*self.batch_size_gauss + j])
    ...
    def _on_rollout_end(self) -> bool:
        ...
        if self.state == "hebo":
            ...
            file_path = f"{REPO_ROOT}/outputs/robots/{self.mat_file_name}_{self.hebo_init_ctr}.mat"
            savemat(file_path, output_data)
        ...
        if self.state == "gauss":
            ...
            for i in range(self.n_envs_train//self.batch_size_gauss):
                ...
                self.logger_reward.append(self.design_rewards_avg[i])
                self.logger_episode_length.append(self.episode_length_avg[i])
                current_arm_length = self.training_env.env_method('get_arm_length', indices=[i*self.batch_size_gauss])[0]
                ...
                if self.design_rewards_avg[i] > self.best_design_reward_gauss:
                    ...
                    self.best_design_gauss = current_arm_length
            ...
            if np.mean(self.design_rewards_avg) >= self.logger_reward_prev:
                self.model.save(f"{REPO_ROOT}/python/forward_dynamics_robot_arm/rl/trained_model/{self.model_name}_{self.gauss_init_ctr}")
