...
def main():
    ...
    while True:
        ...
        env_fns = [lambda config=config: Robot_Chain_Env(**config) for config in env_configs]

        # Create the vectorized environment using SubprocVecEnv directly
        vec_env = SubprocVecEnv(env_fns, start_method='fork')
        model_name = f"ScaraRobot_dynamics_{CALL_BACK_FUNC}_start_upd_{n_update_dist}lr_mean{lr_std_schaff}_std_{lr_std_schaff}_sde{use_sde}_Tanh_Tsteps_{total_timesteps_train}_lr_{learning_rate_train}_hidden_sizes_{hidden_sizes_train}_POSreward_{REWARD[0]}_VELreward_{REWARD[1]}_omega_pen_{REWARD[2]}_var_design"
        log_dir = f"{REPO_ROOT}/python/robot_chain/PlanarRobot_tensorboard/TB_{model_name}"

        if LOAD_OLD_MODEL is True:
            new_model = []
            old_model = PPO.load(f"{REPO_ROOT}/python/robot_chain/rl/trained_model/ScaraRobot_doneTrue_Hebo_Gauss_callback_start_upd_25lr_mean0.01_std_0.01_sdeFalse_Tanh_Tsteps_40960000_lr_0.0001_hidden_sizes_256_POSreward_1.0_VELreward_0.0_omega_pen_0.0_var_design_2", env = vec_env)
        ...
        new_model.save(f"{REPO_ROOT}/python/robot_chain/rl/trained_model/random_design/{model_name}")
        print("Model saved")
        LOAD_OLD_MODEL = True
        vec_env.close()
        break
...
class constant_design(BaseCallback):
    ...
    def _on_rollout_end(self) -> bool:
        self.model.save(f"{REPO_ROOT}/python/robot_chain/rl/trained_model/random_design/{self.model_name}")
    ...
    def _on_step(self) -> bool:
        ...
        if self.num_timesteps > self.steps_update_distribution:
            ...
            if self.num_timesteps % self.save_recorded_data == 0:
                ...
                self.mat_iter += 1
                file_path = f"{REPO_ROOT}/outputs/robot_chain/{self.mat_file_name}_{self.mat_iter}.mat"
                savemat(file_path, output_data)
                self.model.save(f"{REPO_ROOT}/python/robot_chain/rl/trained_model/{self.model_name}_{self.mat_iter}")
...
class Hebo_Gauss_callback(BaseCallback):
    ...
    def __init__(self,model_name = f"matfile", model = None ,n_steps_train=512 * 10, n_envs_train=8, arm_length_limits = np.array([1.0, 5.0]), verbose=0):
        ...
        self.sampling_hebo_time_period = 10
        ...
        self.design_space_ub = np.array([1.5, 1.5, 1.5])
    ...
    def _on_rollout_start(self) -> bool:
        ...
        if self.state == "gauss_init":
            ...
            top_designs = self.hebo_design_history[top_indices]
            ...
            clusters = self.optimal_gmm.predict(top_designs_scaled)
            ...
            # scatter = ax.scatter(top_designs[:, 0], top_designs[:, 1], top_designs[:, 2], c=clusters, cmap='viridis', marker='o')
    ...
    def _on_rollout_end(self) -> bool:
        ...
        if self.state == "gauss":
            ...
            if np.mean(self.design_rewards_avg) > self.logger_reward_prev:
                self.model.save(f"{REPO_ROOT}/python/robot_chain/rl/trained_model/{self.model_name}_{self.gauss_init_ctr}")
