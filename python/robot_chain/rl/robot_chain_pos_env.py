...
class Robot_Chain_Env(utils.EzPickle, gym.Env):
    ...
    def __init__(self, REWARD = np.array([0.7, 0.0, 0.3]), env_id = 0, arm_length = [1.0, 1.0], num_links=8 ,call_back = f"random_design"):
        ...
        self.observation_num = 4 * num_links + 3
        ...
        self.max_length = 1.5*np.ones(self.num_links, dtype=np.double)
    ...
    def reset(self, seed=None):
        ...
        if self.call_back == "random_design":
            arm_length_temp = [random.uniform(self.min_length[0], self.max_length[0]) for _ in range(self.num_links//2)]
            self.arm_length = [1.0, 1.0,
                               arm_length_temp[1], arm_length_temp[1],
                               arm_length_temp[2], arm_length_temp[2],
                               arm_length_temp[3], arm_length_temp[3]]
    ...
    def collision_geometry_maze(self, length, start_x=0, start_y=0):
        ...
        points = [
            (start_x + 1.5 * length, 1),
            (start_x + 1.5 * length, 2),
            (start_x + 2*length, 2),
            (start_x + 2*length, 1),
