...
class AntEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        env_id=0,
        render_mode="None",
        xml_file=f"{REPO_ROOT}/assets/mujoco/ant/ant.xml",
        ...
        **kwargs,
        ...
        self.env_id = env_id
        self.limb_length = np.ones(25)
        xml_file = f"{REPO_ROOT}/assets/mujoco/ant/ant_{self.env_id}.xml"
    ...
    def is_healthy(self):
        ...
        # # convert in euler angles using quaternions
        # # Create a Rotation object from the quaternion
        # position = self.data.qpos.flat.copy()
        # orientation = np.array([position[6], position[3], position[4], position[5]])
        ...
        # #unhealthy if the ant is not standing up
        # threshold_angle = 90
        # is_healthy = True
        # if abs(euler_angles[0]) < threshold_angle or abs(euler_angles[1]) > threshold_angle:
        #     is_healthy = False
    ...
    def _get_obs(self):
        ...
        if self._use_contact_forces:
            ...
            return np.concatenate((position, velocity, contact_force, limb_length))
        else:
            return np.concatenate((position, velocity, limb_length))
