...
def state_space(m, x_cg, x_ea, k_h, k_a):
    rho = 1
    c = 1
    mu = 5
    r_a = 0.25 * c
    ...
    CMd2nc = -(1 / 4) * ((1 / 4 - nu_fh) * (np.pi - theta_fh) + (2 / 3 - (5 / 12) * nu_fh + (1 / 3) * nu_fh ** 2 + (1 / 6) * nu_fh ** 3) * np.sin(theta_fh))
    ...
    M = np.array([[m, m * (x_cg - x_ea) / (c / 2)], [m * (x_cg - x_ea) / (c / 2), m * r_a ** 2 / (c ** 2 / 4)]])
    K = np.array([[k_h, 0], [0, k_a / (c ** 2 / 4)]])

    M_ae = (4 * Vinf ** 2 / c ** 2) * M - qinf * kappa_g * A_2[:, :2]
    invM = np.linalg.inv(M_ae)

    Cae = - qinf * np.dot(kappa_g, (A_1 - (1 / 2) * A_3))
    K_ae = - qinf * np.dot(kappa_g, A_0)
    K_ae[:2, :2] += K

    # Constructing A_ae, B_ae, C_ae matrices
    A_ae = np.block([[np.zeros((3, 3)), np.eye(3), np.zeros((3, 3 * Na))],
                     [-np.dot(invM, K_ae), -np.dot(invM, Cae), qinf * np.dot(np.dot(invM, kappa_g), C_a)],
                     [np.zeros((1, 3)), np.zeros((1, 3)), np.zeros((1, 3 * Na))],
                     [np.zeros((3 * Na, 3)), B_a, A_a]])
    prod = np.dot(invM, kappa_g)
    B_ae = np.block([[np.zeros((2, 1))],
                     [-qinf * np.dot(prod, A_0[:,2:3])],
                     [np.zeros((3 * Na + 2, 1))]])


    C_ae = np.block([A_0[0,:], 0, A_1[0, 1:3], np.zeros((1, 3 * Na))])
    V_ae = np.linalg.eigvals(A_ae)
