...
class PlanarRobotEnv(utils.EzPickle, gym.Env):
    ...
    def __init__(self, REWARD = np.array([0.7, 0.0, 0.3]), env_id = 0, arm_length = [1.0, 1.0, 1.0], call_back = f"random_design"):
        ...
        self.arm_length_sum = np.sum(self.arm_length)
        ...
        self.min_joint_vel = np.array([-20.0, -20.0, -20.0], dtype=np.double)
        self.max_joint_vel = np.array([20.0, 20.0, 20.0], dtype=np.double)
        self.max_torque = np.array([10.0, 10.0, 10.0], dtype=np.double)
        self.min_torque = np.array([-10.0, -10.0, -10.0], dtype=np.double)
        ...
        self.target_threshold = self.convert_range(0.5, 0, 2*self.arm_length_sum, 0, 1)
        ...
        self.filter = FirstOrderDelay(alpha=1)
    ...
    def step(self, action):
        ...
        self.joint_torque = self.filter.step(joint_torque_real)
        ...
        input_penalty = np.linalg.norm(np.average(abs(action)))

        reward = self.TARGET_COEFF * reward_target - self.ACTION_PENALTY_COEFF * input_penalty
        # + self.ACTION_PENALTY_COEFF * input_penalty
        # if self.env_id == 0:
        #     print(f"target_dist: {target_dist:<7.2f}, dist: {np.linalg.norm(self.target_pos_tcp - self.pos_tcp):<7.2f}, reward_tar: {reward_target:<7.2f}, reward_input: {input_penalty:<7.2f}, reward_total: {reward:<7.2f}")
        ...
        if (self.curr_joint_pos[0] > self.max_joint_pos[0] or self.curr_joint_pos[0] < self.min_joint_pos[0] or
            self.curr_joint_pos[1] > self.max_joint_pos[1] or self.curr_joint_pos[1] < self.min_joint_pos[1] or
            self.curr_joint_pos[2] > self.max_joint_pos[2] or self.curr_joint_pos[2] < self.min_joint_pos[2]):
            done = True
            early_Stop = True
            reward = -1.0
            reward = float(reward)
        ...
        if (np.any(np.isnan(obs_tcp_pos)) or np.any(np.isinf(obs_tcp_pos)) or
            ...
            print("nan or inf detected in step")
            observation = np.ones(14)
            reward = -1.0
            reward = float(reward)
            done = True
            early_Stop = True
    ...
    def reset(self, seed=None):
        ...
        self.target_pos_tcp = np.array([random.uniform(self.pos_tcp_range_x[0], self.pos_tcp_range_x[1]),
                                        random.uniform(self.pos_tcp_range_y[0], self.pos_tcp_range_y[1]),
                                        0],
                                        dtype=np.double)

        self.curr_joint_pos = np.array([random.uniform(self.min_joint_pos[0], self.max_joint_pos[0]),
                                        random.uniform(self.min_joint_pos[1], self.max_joint_pos[1]),
                                        random.uniform(self.min_joint_pos[2], self.max_joint_pos[2])],
                                        dtype=np.double)
        # self.target_pos_tcp = np.array([-3.0, 0.0, 0.0], dtype=np.double)
        # self.curr_joint_pos = np.array([0.0, 0.0, 0.0], dtype=np.double)
        self.vel_tcp = np.array([0.0, 0.0, 0.0], dtype=np.double)
        # Call the wrapped function
        # self.model.calc_sys_matrices(self.curr_joint_pos, self.curr_joint_vel,
