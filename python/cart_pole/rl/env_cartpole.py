...
class CartPoleEnv(utils.EzPickle, gym.Env):
    ...
    def __init__(self, REWARD = np.array([0.0, 0.0, 0.0]), env_id = 0, pole_length = 1.0,init_pos = 0, freq=1 ,call_back = f"constant_design_eval"):
        super(CartPoleEnv, self).__init__()

        utils.EzPickle.__init__(self)
        self.call_back = call_back
        self.action_space = Box(low=-1, high=1, shape=(1,))
        if self.call_back == "constant_design" or self.call_back == "constant_design_eval":
    ...
    def step(self, action):
        ...
        reward = float(reward)
        return observation, reward, done, early_Stop, {"cart_pos": self.cart_pos, "pole_pos":self.pole_pos, "pole_length":self.pole_length, "effort":self.effort, "pole_vel":self.pole_vel, "cart_vel":self.cart_vel, "pole_angle":self.pole_angle, "force":action, "init_pos":self.init_pole_pos}
    ...
    def reset(self, seed=None):
        ...
        if self.call_back == "random_design":
            ...
            self.init_pole_pos = random.uniform(-m.pi+0.001, m.pi-0.001)

        elif self.call_back == "constant_design":
            self.init_pole_pos = random.uniform(-m.pi+0.001, m.pi-0.001)
    ...
    def set_init_pole_pos(self, init_pole_pos):
        self.init_pole_pos = init_pole_pos

    def get_init_pole_pos(self):
        return self.init_pole_pos
