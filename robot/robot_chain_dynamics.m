...
function [joint_acc, pos_tcp, ROBOT] = robot_chain_dynamics(q, dq, rho, num_links, radius, arm_length, torque)
%#codegen
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ROBOT = Planar_n_DoF(rho, radius, arm_length, num_links);
...
function ROBOT = Planar_n_DoF(rho, radius, arm_length, num_links)
...
jointOrigins = zeros(3,num_links);
for i = 2:num_links
    jointOrigins(:,i) = [arm_length(i-1) 0 0]';
end

% Parameter of end-effector
TCP_T_EE = [E, [arm_length(num_links-1) 0 0]'; o.', 1]; % homogeneous transformation of tcp

% Mass, Center of Mass, and Inertia calculations for each link
Mass = cell(num_links,1);
g0 = cell(num_links,1);
for iLink = 1:num_links
...
% Relative body jacobians adjusted for n DOF planar robot (rotation around z-axis)
% ROBOT.csi = repmat([o; wz], num_links, 1)
...
ROBOT.g0 = zeros(4 * num_links, 4);
ROBOT.Mass = zeros(6 * num_links, 6);
...
for i = 1:num_links
    ...
    g0Size = size(g0{i}, 1);
    massSize = size(Mass{i}, 1);

    % Fill the pre-allocated arrays
    ROBOT.g0(currentG0Index:currentG0Index+g0Size-1, :) = g0{i};
    ROBOT.Mass(currentMassIndex:currentMassIndex+massSize-1, :) = Mass{i};

    % Update the indices
    currentG0Index = currentG0Index + g0Size;
    currentMassIndex = currentMassIndex + massSize;
end
