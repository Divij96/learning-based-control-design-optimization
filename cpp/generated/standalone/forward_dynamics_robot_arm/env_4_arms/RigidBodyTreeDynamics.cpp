...
int RigidBodyTreeDynamics::inverseDynamics(
    RigidBodyTree &robot, const double q_data[], const double qdot_data[],
    const ::coder::array<double, 2U> &fext, double tau_data[])
{
  static const char b_cv[5]{'f', 'i', 'x', 'e', 'd'};
  b_RigidBody *obj;
  rigidBodyJoint *jnt;
  array<cell_wrap_30, 2U> X;
  array<cell_wrap_30, 2U> Xtree;
  array<double, 2U> S;
  array<double, 2U> aB;
  array<double, 2U> b_y;
  array<double, 2U> f;
  ...
  for (int b_i{0}; b_i < i; b_i++) {
    ...
    a_idx_1 = robot.PositionDoFMap[b_i + 5];
    b_idx_0 = robot.VelocityDoFMap[b_i];
    b_idx_1 = robot.VelocityDoFMap[b_i + 5];
    std::memset(&XDHOffset[0], 0, 36U * sizeof(double));
    for (int k{0}; k < 6; k++) {
      XDHOffset[k + 6 * k] = 1.0;
    }
    if (a_idx_1 < a_idx_0) {
      ...
      for (int k{0}; k < loop_ub_tmp; k++) {
        ...
        for (int d_i{0}; d_i < 6; d_i++) {
          vJ[d_i + 6 * b_i] =
              vJ[d_i + 6 * b_i] + b_y[aoffset + d_i] * qdot_data[i1 + k];
        }
      }
    }
    for (j = 0; j < 3; j++) {
    ...
    for (j = 0; j < 6; j++) {
      ...
      a_idx_1 = 0.0;
      for (aoffset = 0; aoffset < 6; aoffset++) {
        a_idx_1 += d_R[j + 6 * aoffset] * y[aoffset];
  ...
  for (int b_i{0}; b_i < i; b_i++) {
    ...
    if (!b_bool) {
      ...
      b_idx_1 = robot.VelocityDoFMap[static_cast<int>(c_i) + 4];
      if (b_idx_0 > b_idx_1) {
        j = 0;
        aoffset = 0;
      } else {
    ...
    if (b_idx_1 > 0.0) {
      for (j = 0; j < 6; j++) {
        ...
        y[j] = f[j + 6 * (static_cast<int>(b_idx_1) - 1)] + a_idx_0;
      }
      for (j = 0; j < 6; j++) {
        f[j + 6 * (static_cast<int>(b_idx_1) - 1)] = y[j];
      }
    }
  }
  return tau_size;
...
// Return Type  : void
//
void RigidBodyTreeDynamics::massMatrix(RigidBodyTree &robot,
                                       const double q_data[],
                                       ::coder::array<double, 2U> &H,
                                       ::coder::array<double, 2U> &lambda)
{
  ...
  for (int b_i{0}; b_i < loop_ub_tmp; b_i++) {
    ...
    p_idx_1 = robot.PositionDoFMap[b_i + 5];
    if (p_idx_1 < p_idx_0) {
      obj = robot.Bodies[b_i];
      jnt = obj->JointInternal;
      jnt->transformBodyToParent(T);
  ...
  for (int b_i{0}; b_i < i; b_i++) {
    ...
    p_idx_1 = robot.VelocityDoFMap[static_cast<int>(c_i) + 4];
    if (pid > 0.0) {
      for (i1 = 0; i1 < 6; i1++) {
        for (k = 0; k < 6; k++) {
          vNum = 0.0;
    ...
    if (b_idx_0 <= b_idx_1) {
      ...
      while (pid > 0.0) {
        ...
        b_idx_1 = robot.VelocityDoFMap[static_cast<int>(pid) + 4];
        if (b_idx_0 <= b_idx_1) {
          R[0] = 0.0;
          R[3] = -c_R[2];
          R[6] = c_R[1];
  ...
  for (i = 0; i < 5; i++) {
    mask[i] = (robot.VelocityDoFMap[i] <= robot.VelocityDoFMap[i + 5]);
  }
  ...
  for (k = 0; k < loop_ub_tmp; k++) {
    ...
    p_idx_1 = robot.VelocityDoFMap[nm1d2 + 4];
    ...
    if (vNum == 0.0) {
      ...
      b_idx_1 = robot.VelocityDoFMap[static_cast<int>(vNum) + 4];
