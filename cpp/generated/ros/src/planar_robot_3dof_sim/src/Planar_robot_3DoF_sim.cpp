 ...
 * solver algorithm
 */
void Planar_robot_3DoF_sim::rt_ertODEUpdateContinuousStates(RTWSolverInfo *si )
{
  /* Solver Matrices */
  ...
  /* f0 = f(t,y) */
  rtsiSetdX(si, f0);
  Planar_robot_3DoF_sim_derivatives();
...
/* Function for MATLAB Function: '<S2>/calcSysMatrices' */
  ...
  const real_T in6[6], const real_T in7[18])
{
  ...
  /* Outputs for Atomic SubSystem: '<S1>/planar_robot3dof_FD' */
  /* MATLAB Function: '<S2>/calcSysMatrices' */
  in4_size[0] = 6;
  in4_size[1] = in2;
  for (i = 0; i < in2; i++) {
  ...
  /* End of MATLAB Function: '<S2>/calcSysMatrices' */
  /* End of Outputs for SubSystem: '<S1>/planar_robot3dof_FD' */
}
...
{
  if (rtmIsMajorTimeStep((&Planar_robot_3DoF_sim_M))) {
    /* set solver stop time */
    rtsiSetSolverStopTime(&(&Planar_robot_3DoF_sim_M)->solverInfo,
                          (((&Planar_robot_3DoF_sim_M)->Timing.clockTick0+1)*
      (&Planar_robot_3DoF_sim_M)->Timing.stepSize0));
  }                                    /* end MajorTimeStep */

  /* Update absolute time of base rate at minor time step */
  if (rtmIsMinorTimeStep((&Planar_robot_3DoF_sim_M))) {
    (&Planar_robot_3DoF_sim_M)->Timing.t[0] = rtsiGetT
      (&(&Planar_robot_3DoF_sim_M)->solverInfo);
  }

  {
    ...
    Planar_robot_3DoF_sim_B.j2_torque = std::sin(13.0 *
      (&Planar_robot_3DoF_sim_M)->Timing.t[0]) * 0.0;
    ...
    if (rtmIsMajorTimeStep((&Planar_robot_3DoF_sim_M))) {
      ...
      Planar_robot_3DoF_sim_B.joint_torque[0] =
        Planar_robot_3DoF_sim_B.j1_torque;
      Planar_robot_3DoF_sim_B.joint_torque[1] =
        Planar_robot_3DoF_sim_B.j2_torque;
      Planar_robot_3DoF_sim_B.joint_torque[2] =
        Planar_robot_3DoF_sim_B.j3_torque;
    }

    /* Outputs for Atomic SubSystem: '<S1>/planar_robot3dof_FD' */
    /* MATLAB Function: '<S2>/calcSysMatrices' incorporates:
    ...
    inertialTwist_pre[0] = 1.5707963267948967E-5;
    inertialTwist_pre[1] = 0.026187792761548916;
    inertialTwist_pre[2] = 0.026187792761548916;
    ...
    /* :          -vec(2),   vec(1),    0 */
    /* :      ]; */
    Planar_robot_3DoF_sim_B.q[0] = Planar_robot_3DoF_sim_X.Integrator1_CSTATE[0];

    /* Integrator: '<S2>/Integrator' */
    Planar_robot_3DoF_sim_B.dq[0] = Planar_robot_3DoF_sim_X.Integrator_CSTATE[0];
    ...
    /* Integrator: '<S2>/Integrator1' */
    Planar_robot_3DoF_sim_B.q[1] = Planar_robot_3DoF_sim_X.Integrator1_CSTATE[1];

    /* Integrator: '<S2>/Integrator' */
    Planar_robot_3DoF_sim_B.dq[1] = Planar_robot_3DoF_sim_X.Integrator_CSTATE[1];
    ...
    /* Integrator: '<S2>/Integrator1' */
    Planar_robot_3DoF_sim_B.q[2] = Planar_robot_3DoF_sim_X.Integrator1_CSTATE[2];

    /* Integrator: '<S2>/Integrator' */
    Planar_robot_3DoF_sim_B.dq[2] = Planar_robot_3DoF_sim_X.Integrator_CSTATE[2];
    ...
    /* :  body.Inertia = [Ix, Iz, Iz, 0, 0, 0]; */
    /* :  Mass{iLink} = Inertia(body.Mass, body.CenterOfMass, diag(body.Inertia(1:3))); */
    inertialTwist_pre[0] = 1.5707963267948969E-6;
    inertialTwist_pre[1] = 2.69653369433124E-5;
    inertialTwist_pre[2] = 2.69653369433124E-5;
    inertialTwist_pre[3] = 0.0;
    inertialTwist_pre[4] = 0.0;
    inertialTwist_pre[5] = 0.0;
    ...
    for (i = 0; i < 9; i++) {
      c_b = c_b_0[i];
      y[i] = 0.31415926535897931 * static_cast<real_T>(c_b);
      b_y[i] = 0.31415926535897931 * S[i];
      c_y[i] = 0.031415926535897934 * static_cast<real_T>(c_b);
      d_y[i] = 0.031415926535897934 * b_S[i];
    }
    ...
    for (i = 0; i < 3; i++) {
      for (O_Htm_tmp = 0; O_Htm_tmp < 3; O_Htm_tmp++) {
        ROBOT_csi_1 = 0.31415926535897931 * S[i] * S[3 * O_Htm_tmp];
        Iz = 0.031415926535897934 * b_S[i] * b_S[3 * O_Htm_tmp];
        m = 0.031415926535897934 * c_S[i] * c_S[3 * O_Htm_tmp];
        ROBOT_Mass_tmp = 3 * O_Htm_tmp + 1;
        ROBOT_csi_1 += S[i + 3] * 0.31415926535897931 * S[ROBOT_Mass_tmp];
        Iz += b_S[i + 3] * 0.031415926535897934 * b_S[ROBOT_Mass_tmp];
        m += c_S[i + 3] * 0.031415926535897934 * c_S[ROBOT_Mass_tmp];
        ROBOT_Mass_tmp = 3 * O_Htm_tmp + 2;
        ROBOT_csi_1 += S[i + 6] * 0.31415926535897931 * S[ROBOT_Mass_tmp];
        Iz += b_S[i + 6] * 0.031415926535897934 * b_S[ROBOT_Mass_tmp];
        m += c_S[i + 6] * 0.031415926535897934 * c_S[ROBOT_Mass_tmp];
    ...
    for (i = 0; i < 3; i++) {
      ...
      ROBOT_Mass[idxStart_4Row + 6] = b_S[3 * i] * -0.031415926535897934;
      ROBOT_Mass[18 * i + 9] = d_y[3 * i];
      ROBOT_Mass[idxStart_4Row + 9] = b_I_0[3 * i];
      ROBOT_Mass[18 * i + 12] = static_cast<real_T>(c_b_0[3 * i]) *
        0.031415926535897934;
      Iz = c_S[3 * i];
      ROBOT_Mass[idxStart_4Row + 12] = -0.031415926535897934 * Iz;
      ROBOT_Mass[18 * i + 15] = 0.031415926535897934 * Iz;
      ...
      ROBOT_Mass[idxStart_4Row + 13] = -0.031415926535897934 * Iz;
      ROBOT_Mass[18 * i + 16] = 0.031415926535897934 * Iz;
      ...
      ROBOT_Mass[18 * i + 14] = static_cast<real_T>(c_b_0[ROBOT_Mass_tmp]) *
        0.031415926535897934;
      Iz = c_S[ROBOT_Mass_tmp];
      ROBOT_Mass[idxStart_4Row + 14] = -0.031415926535897934 * Iz;
      ROBOT_Mass[18 * i + 17] = 0.031415926535897934 * Iz;
    ...
    for (ROBOT_Mass_tmp = 0; ROBOT_Mass_tmp < 3; ROBOT_Mass_tmp++) {
      ...
      for (i = 0; i < 6; i++) {
        relTwist[i] = ROBOT_csi[idxStart_6Row + i] * ROBOT_csi_1;
      }
      ...
      /* :  O_Htm = O_Htm_pre * Htm; */
      for (i = 0; i < 4; i++) {
        ROBOT_csi_1 = O_Htm_pre[i];
        m = O_Htm_pre[i + 4];
        O_Htm_pre_0 = O_Htm_pre[i + 8];
        O_Htm_pre_1 = O_Htm_pre[i + 12];
        for (O_Htm_tmp = 0; O_Htm_tmp < 4; O_Htm_tmp++) {
          O_Htm_tmp_0 = O_Htm_tmp << 2;
          Iz = Htm_data[O_Htm_tmp_0] * ROBOT_csi_1;
          Iz += Htm_data[O_Htm_tmp_0 + 1] * m;
          Iz += Htm_data[O_Htm_tmp_0 + 2] * O_Htm_pre_0;
          Iz += Htm_data[O_Htm_tmp_0 + 3] * O_Htm_pre_1;
          O_Htm[i + O_Htm_tmp_0] = Iz;
        }
      }
      ...
      if (ROBOT_Mass_tmp + 1 > 1) {
        ...
        for (i = 0; i < 6; i++) {
          ...
          for (O_Htm_tmp = 0; O_Htm_tmp < 6; O_Htm_tmp++) {
            ROBOT_csi_1 += invAd[6 * O_Htm_tmp + i] *
              inertialTwist_pre[O_Htm_tmp];
          }
        ...
        if (Htm_size[1] == tmp_size[1]) {
          ...
          Planar_robot_3DoF_sim_mtimes_p(invAd, J_pre_data, J_pre_size,
            tmp_data_1, Htm_size);
          jointOrigins_0[18] = 0.0;
          jointOrigins_0[24] = -relTwist[2];
          jointOrigins_0[30] = relTwist[1];
      ...
      if ((ROBOT_Mass_tmp + 1 == tmp_size[0]) && (ROBOT_Mass_tmp + 1 ==
        ...
        Planar_robo_binary_expand_op_cc(Planar_robot_3DoF_sim_B.M,
      ...
      for (i = 0; i < 3; i++) {
        ...
        O_Htm_pre_1 = ROBOT_Mass[idxStart_4Row + 72];
      ...
      if ((ROBOT_Mass_tmp + 1 == tmp_size[0]) && (ROBOT_Mass_tmp + 1 ==
        ...
        for (i = 0; i <= ROBOT_Mass_tmp; i++) {
          for (O_Htm_tmp = 0; O_Htm_tmp <= ROBOT_Mass_tmp; O_Htm_tmp++) {
            b_I[O_Htm_tmp + tmp_size_1[0] * i] = (Planar_robot_3DoF_sim_B.CC[3 *
              i + O_Htm_tmp] + y[tmp_size[0] * i + O_Htm_tmp]) + b_y[tmp_size_0
              [0] * i + O_Htm_tmp];
          }
        }
        ...
        for (i = 0; i < O_Htm_tmp_0; i++) {
          for (O_Htm_tmp = 0; O_Htm_tmp < idxStart_4Row; O_Htm_tmp++) {
            Planar_robot_3DoF_sim_B.CC[O_Htm_tmp + 3 * i] = b_I[tmp_size_1[0] *
              i + O_Htm_tmp];
          }
        }
      } else {
        Planar_robot_binary_expand_op_c(Planar_robot_3DoF_sim_B.CC,
      ...
      if (ROBOT_Mass_tmp + 1 == i) {
        ...
        Planar_robot_3_binary_expand_op(Planar_robot_3DoF_sim_B.g,
    ...
    for (i = 0; i < 4; i++) {
      ROBOT_Mass_tmp = i << 2;
      ROBOT_csi_1 = O_Htm_pre[ROBOT_Mass_tmp];
      m = O_Htm_pre[ROBOT_Mass_tmp + 1];
      O_Htm_pre_0 = O_Htm_pre[ROBOT_Mass_tmp + 2];
      O_Htm_pre_1 = O_Htm_pre[ROBOT_Mass_tmp + 3];
      for (O_Htm_tmp = 0; O_Htm_tmp < 4; O_Htm_tmp++) {
        Iz = ROBOT_csi_1 * O_Htm[O_Htm_tmp];
        Iz += O_Htm[O_Htm_tmp + 4] * m;
        Iz += O_Htm[O_Htm_tmp + 8] * O_Htm_pre_0;
        Iz += O_Htm[O_Htm_tmp + 12] * O_Htm_pre_1;
        O_Htm_0[O_Htm_tmp + ROBOT_Mass_tmp] = Iz;
      }
    }

    Planar_robot_3DoF_sim_B.pos_tcp[0] = O_Htm_0[12];
    Planar_robot_3DoF_sim_B.pos_tcp[1] = O_Htm_0[13];
    Planar_robot_3DoF_sim_B.pos_tcp[2] = O_Htm_0[14];
    ...
    if ((Planar_robot_3DoF_sim_DW.TimeStampA >= (&Planar_robot_3DoF_sim_M)
         ->Timing.t[0]) && (Planar_robot_3DoF_sim_DW.TimeStampB >=
                            (&Planar_robot_3DoF_sim_M)->Timing.t[0])) {
      /* Derivative: '<S2>/Derivative1' */
      Planar_robot_3DoF_sim_B.Derivative1[0] = 0.0;
      Planar_robot_3DoF_sim_B.Derivative1[1] = 0.0;
      Planar_robot_3DoF_sim_B.Derivative1[2] = 0.0;
    } else {
      m = Planar_robot_3DoF_sim_DW.TimeStampA;
      lastU = &Planar_robot_3DoF_sim_DW.LastUAtTimeA;
      if (Planar_robot_3DoF_sim_DW.TimeStampA <
          Planar_robot_3DoF_sim_DW.TimeStampB) {
        if (Planar_robot_3DoF_sim_DW.TimeStampB < (&Planar_robot_3DoF_sim_M)
            ->Timing.t[0]) {
          m = Planar_robot_3DoF_sim_DW.TimeStampB;
          lastU = &Planar_robot_3DoF_sim_DW.LastUAtTimeB;
        }
      } else if (Planar_robot_3DoF_sim_DW.TimeStampA >=
                 (&Planar_robot_3DoF_sim_M)->Timing.t[0]) {
        m = Planar_robot_3DoF_sim_DW.TimeStampB;
        lastU = &Planar_robot_3DoF_sim_DW.LastUAtTimeB;
      }

      m = (&Planar_robot_3DoF_sim_M)->Timing.t[0] - m;

      /* Derivative: '<S2>/Derivative1' */
      Planar_robot_3DoF_sim_B.Derivative1[0] = (Planar_robot_3DoF_sim_B.pos_tcp
        [0] - (*lastU)[0]) / m;
      Planar_robot_3DoF_sim_B.Derivative1[1] = (Planar_robot_3DoF_sim_B.pos_tcp
        [1] - (*lastU)[1]) / m;
      Planar_robot_3DoF_sim_B.Derivative1[2] = (Planar_robot_3DoF_sim_B.pos_tcp
        [2] - (*lastU)[2]) / m;
    }
    ...
    if ((Planar_robot_3DoF_sim_DW.TimeStampA_f >= (&Planar_robot_3DoF_sim_M)
         ->Timing.t[0]) && (Planar_robot_3DoF_sim_DW.TimeStampB_b >=
                            (&Planar_robot_3DoF_sim_M)->Timing.t[0])) {
      /* Derivative: '<S2>/Derivative' */
      Planar_robot_3DoF_sim_B.Derivative[0] = 0.0;
      Planar_robot_3DoF_sim_B.Derivative[1] = 0.0;
      Planar_robot_3DoF_sim_B.Derivative[2] = 0.0;
    } else {
      m = Planar_robot_3DoF_sim_DW.TimeStampA_f;
      lastU = &Planar_robot_3DoF_sim_DW.LastUAtTimeA_n;
      if (Planar_robot_3DoF_sim_DW.TimeStampA_f <
          Planar_robot_3DoF_sim_DW.TimeStampB_b) {
        if (Planar_robot_3DoF_sim_DW.TimeStampB_b < (&Planar_robot_3DoF_sim_M)
            ->Timing.t[0]) {
          m = Planar_robot_3DoF_sim_DW.TimeStampB_b;
          lastU = &Planar_robot_3DoF_sim_DW.LastUAtTimeB_j;
        }
      } else if (Planar_robot_3DoF_sim_DW.TimeStampA_f >=
                 (&Planar_robot_3DoF_sim_M)->Timing.t[0]) {
        m = Planar_robot_3DoF_sim_DW.TimeStampB_b;
        lastU = &Planar_robot_3DoF_sim_DW.LastUAtTimeB_j;
      }
      ...
      /* Derivative: '<S2>/Derivative' */
      Planar_robot_3DoF_sim_B.Derivative[0] =
        (Planar_robot_3DoF_sim_B.Derivative1[0] - (*lastU)[0]) / m;
      Planar_robot_3DoF_sim_B.Derivative[1] =
        (Planar_robot_3DoF_sim_B.Derivative1[1] - (*lastU)[1]) / m;
      Planar_robot_3DoF_sim_B.Derivative[2] =
        (Planar_robot_3DoF_sim_B.Derivative1[2] - (*lastU)[2]) / m;
    }
    ...
    if (rtmIsMajorTimeStep((&Planar_robot_3DoF_sim_M))) {
      ...
      Planar_robot_3DoF_sim_B.TmpSignalConversionAtToWorkspac[12] =
        Planar_robot_3DoF_sim_B.Derivative1[0];
      ...
      Planar_robot_3DoF_sim_B.TmpSignalConversionAtToWorkspac[13] =
        Planar_robot_3DoF_sim_B.Derivative1[1];
      ...
      Planar_robot_3DoF_sim_B.TmpSignalConversionAtToWorkspac[14] =
        Planar_robot_3DoF_sim_B.Derivative1[2];
  ...
  if (rtmIsMajorTimeStep((&Planar_robot_3DoF_sim_M))) {
    ...
    /* Update for Atomic SubSystem: '<S1>/planar_robot3dof_FD' */
    /* Update for Derivative: '<S2>/Derivative1' */
    if (Planar_robot_3DoF_sim_DW.TimeStampA == (rtInf)) {
      Planar_robot_3DoF_sim_DW.TimeStampA = (&Planar_robot_3DoF_sim_M)->
        Timing.t[0];
      lastU = &Planar_robot_3DoF_sim_DW.LastUAtTimeA;
    } else if (Planar_robot_3DoF_sim_DW.TimeStampB == (rtInf)) {
      Planar_robot_3DoF_sim_DW.TimeStampB = (&Planar_robot_3DoF_sim_M)->
        Timing.t[0];
      lastU = &Planar_robot_3DoF_sim_DW.LastUAtTimeB;
    } else if (Planar_robot_3DoF_sim_DW.TimeStampA <
               Planar_robot_3DoF_sim_DW.TimeStampB) {
    ...
    (*lastU)[0] = Planar_robot_3DoF_sim_B.pos_tcp[0];
    (*lastU)[1] = Planar_robot_3DoF_sim_B.pos_tcp[1];
    (*lastU)[2] = Planar_robot_3DoF_sim_B.pos_tcp[2];

    /* End of Update for Derivative: '<S2>/Derivative1' */

    /* Update for Derivative: '<S2>/Derivative' */
    if (Planar_robot_3DoF_sim_DW.TimeStampA_f == (rtInf)) {
      Planar_robot_3DoF_sim_DW.TimeStampA_f = (&Planar_robot_3DoF_sim_M)
        ->Timing.t[0];
      lastU = &Planar_robot_3DoF_sim_DW.LastUAtTimeA_n;
    } else if (Planar_robot_3DoF_sim_DW.TimeStampB_b == (rtInf)) {
      Planar_robot_3DoF_sim_DW.TimeStampB_b = (&Planar_robot_3DoF_sim_M)
        ->Timing.t[0];
      lastU = &Planar_robot_3DoF_sim_DW.LastUAtTimeB_j;
    } else if (Planar_robot_3DoF_sim_DW.TimeStampA_f <
               Planar_robot_3DoF_sim_DW.TimeStampB_b) {
    ...
    (*lastU)[0] = Planar_robot_3DoF_sim_B.Derivative1[0];
    (*lastU)[1] = Planar_robot_3DoF_sim_B.Derivative1[1];
    (*lastU)[2] = Planar_robot_3DoF_sim_B.Derivative1[2];

    /* End of Update for Derivative: '<S2>/Derivative' */
    /* End of Update for SubSystem: '<S1>/planar_robot3dof_FD' */
  }                                    /* end MajorTimeStep */
  ...
  if (rtmIsMajorTimeStep((&Planar_robot_3DoF_sim_M))) {
    ...
    {                                  /* Sample time: [0.0s, 0.0s] */
      if ((rtmGetTFinal((&Planar_robot_3DoF_sim_M))!=-1) &&
          !((rtmGetTFinal((&Planar_robot_3DoF_sim_M))-
             (((&Planar_robot_3DoF_sim_M)->Timing.clockTick1) * 0.001)) >
            (((&Planar_robot_3DoF_sim_M)->Timing.clockTick1) * 0.001) *
            (DBL_EPSILON))) {
    ...
    rt_ertODEUpdateContinuousStates(&(&Planar_robot_3DoF_sim_M)->solverInfo);
     ...
     */
    ++(&Planar_robot_3DoF_sim_M)->Timing.clockTick0;
    (&Planar_robot_3DoF_sim_M)->Timing.t[0] = rtsiGetSolverStopTime
      (&(&Planar_robot_3DoF_sim_M)->solverInfo);
...
void Planar_robot_3DoF_sim::Planar_robot_3DoF_sim_derivatives()
{
  XDot_Planar_robot_3DoF_sim_T *_rtXdot;
  _rtXdot = ((XDot_Planar_robot_3DoF_sim_T *) (&Planar_robot_3DoF_sim_M)->derivs);

  /* Derivatives for Atomic SubSystem: '<S1>/planar_robot3dof_FD' */
  /* Derivatives for Integrator: '<S2>/Integrator1' */
  _rtXdot->Integrator1_CSTATE[0] = Planar_robot_3DoF_sim_B.dq[0];

  /* Derivatives for Integrator: '<S2>/Integrator' */
  _rtXdot->Integrator_CSTATE[0] = Planar_robot_3DoF_sim_B.Product1[0];

  /* Derivatives for Integrator: '<S2>/Integrator1' */
  _rtXdot->Integrator1_CSTATE[1] = Planar_robot_3DoF_sim_B.dq[1];

  /* Derivatives for Integrator: '<S2>/Integrator' */
  _rtXdot->Integrator_CSTATE[1] = Planar_robot_3DoF_sim_B.Product1[1];

  /* Derivatives for Integrator: '<S2>/Integrator1' */
  _rtXdot->Integrator1_CSTATE[2] = Planar_robot_3DoF_sim_B.dq[2];

  /* Derivatives for Integrator: '<S2>/Integrator' */
  _rtXdot->Integrator_CSTATE[2] = Planar_robot_3DoF_sim_B.Product1[2];

  /* End of Derivatives for SubSystem: '<S1>/planar_robot3dof_FD' */
}
...
void Planar_robot_3DoF_sim::initialize()
{
...

  {
    ...
    rtsiSetStepSizePtr(&(&Planar_robot_3DoF_sim_M)->solverInfo,
                       &(&Planar_robot_3DoF_sim_M)->Timing.stepSize0);
    rtsiSetdXPtr(&(&Planar_robot_3DoF_sim_M)->solverInfo,
                 &(&Planar_robot_3DoF_sim_M)->derivs);
    rtsiSetContStatesPtr(&(&Planar_robot_3DoF_sim_M)->solverInfo, (real_T **) &(
      &Planar_robot_3DoF_sim_M)->contStates);
    rtsiSetNumContStatesPtr(&(&Planar_robot_3DoF_sim_M)->solverInfo,
      &(&Planar_robot_3DoF_sim_M)->Sizes.numContStates);
    rtsiSetNumPeriodicContStatesPtr(&(&Planar_robot_3DoF_sim_M)->solverInfo,
      &(&Planar_robot_3DoF_sim_M)->Sizes.numPeriodicContStates);
    rtsiSetPeriodicContStateIndicesPtr(&(&Planar_robot_3DoF_sim_M)->solverInfo,
      &(&Planar_robot_3DoF_sim_M)->periodicContStateIndices);
    rtsiSetPeriodicContStateRangesPtr(&(&Planar_robot_3DoF_sim_M)->solverInfo,
      &(&Planar_robot_3DoF_sim_M)->periodicContStateRanges);
  ...
  (&Planar_robot_3DoF_sim_M)->intgData.y = (&Planar_robot_3DoF_sim_M)->odeY;
  (&Planar_robot_3DoF_sim_M)->intgData.f[0] = (&Planar_robot_3DoF_sim_M)->odeF[0];
  (&Planar_robot_3DoF_sim_M)->intgData.f[1] = (&Planar_robot_3DoF_sim_M)->odeF[1];
  (&Planar_robot_3DoF_sim_M)->intgData.f[2] = (&Planar_robot_3DoF_sim_M)->odeF[2];
  (&Planar_robot_3DoF_sim_M)->contStates = ((X_Planar_robot_3DoF_sim_T *)
    &Planar_robot_3DoF_sim_X);
  rtsiSetSolverData(&(&Planar_robot_3DoF_sim_M)->solverInfo, static_cast<void *>
                    (&(&Planar_robot_3DoF_sim_M)->intgData));
  rtsiSetIsMinorTimeStepWithModeChange(&(&Planar_robot_3DoF_sim_M)->solverInfo,
    false);
  rtsiSetSolverName(&(&Planar_robot_3DoF_sim_M)->solverInfo,"ode3");
  ...
  /* SystemInitialize for Atomic SubSystem: '<S1>/planar_robot3dof_FD' */
  /* InitializeConditions for Integrator: '<S2>/Integrator1' */
  Planar_robot_3DoF_sim_X.Integrator1_CSTATE[0] = 0.0;

  /* InitializeConditions for Integrator: '<S2>/Integrator' */
  Planar_robot_3DoF_sim_X.Integrator_CSTATE[0] = 0.0;

  /* InitializeConditions for Integrator: '<S2>/Integrator1' */
  Planar_robot_3DoF_sim_X.Integrator1_CSTATE[1] = 0.0;

  /* InitializeConditions for Integrator: '<S2>/Integrator' */
  Planar_robot_3DoF_sim_X.Integrator_CSTATE[1] = 0.0;

  /* InitializeConditions for Integrator: '<S2>/Integrator1' */
  Planar_robot_3DoF_sim_X.Integrator1_CSTATE[2] = 0.0;

  /* InitializeConditions for Integrator: '<S2>/Integrator' */
  Planar_robot_3DoF_sim_X.Integrator_CSTATE[2] = 0.0;

  /* InitializeConditions for Derivative: '<S2>/Derivative1' */
  Planar_robot_3DoF_sim_DW.TimeStampA = (rtInf);
  Planar_robot_3DoF_sim_DW.TimeStampB = (rtInf);

  /* InitializeConditions for Derivative: '<S2>/Derivative' */
  Planar_robot_3DoF_sim_DW.TimeStampA_f = (rtInf);
  Planar_robot_3DoF_sim_DW.TimeStampB_b = (rtInf);

  /* End of SystemInitialize for SubSystem: '<S1>/planar_robot3dof_FD' */
}
