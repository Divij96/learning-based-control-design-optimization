    ...
    const ::coder::array<double, 2U> &fext, double tau_data[])
{
  ...
  f.set_size(6, i);
  tau_size = 10;
  std::memset(&tau_data[0], 0, 10U * sizeof(double));
  for (int k{0}; k < i; k++) {
  ...
  for (int b_i{0}; b_i < i; b_i++) {
    ...
    a_idx_1 = robot.PositionDoFMap[b_i + 11];
    b_idx_0 = robot.VelocityDoFMap[b_i];
    b_idx_1 = robot.VelocityDoFMap[b_i + 11];
    std::memset(&XDHOffset[0], 0, 36U * sizeof(double));
    for (int k{0}; k < 6; k++) {
      XDHOffset[k + 6 * k] = 1.0;
    }
  ...
  for (int b_i{0}; b_i < i; b_i++) {
    ...
    if (!b_bool) {
      ...
      b_idx_1 = robot.VelocityDoFMap[static_cast<int>(c_i) + 10];
      if (b_idx_0 > b_idx_1) {
        j = 0;
        aoffset = 0;
      } else {
                                       ...
                                       ::coder::array<double, 2U> &lambda)
{
  ...
  for (int b_i{0}; b_i < loop_ub_tmp; b_i++) {
    ...
    p_idx_1 = robot.PositionDoFMap[b_i + 11];
    if (p_idx_1 < p_idx_0) {
      obj = robot.Bodies[b_i];
      jnt = obj->JointInternal;
      jnt->transformBodyToParent(T);
  ...
  for (int b_i{0}; b_i < i; b_i++) {
    ...
    p_idx_1 = robot.VelocityDoFMap[static_cast<int>(c_i) + 10];
    if (pid > 0.0) {
      for (i1 = 0; i1 < 6; i1++) {
        for (k = 0; k < 6; k++) {
          vNum = 0.0;
    ...
    if (b_idx_0 <= b_idx_1) {
      ...
      while (pid > 0.0) {
        ...
        b_idx_1 = robot.VelocityDoFMap[static_cast<int>(pid) + 10];
        if (b_idx_0 <= b_idx_1) {
          R[0] = 0.0;
          R[3] = -c_R[2];
          R[6] = c_R[1];
  ...
  for (i = 0; i < 11; i++) {
    mask[i] = (robot.VelocityDoFMap[i] <= robot.VelocityDoFMap[i + 11]);
  }
  ...
  for (k = 0; k < loop_ub_tmp; k++) {
    ...
    p_idx_1 = robot.VelocityDoFMap[nm1d2 + 10];
    ...
    if (vNum == 0.0) {
      ...
      b_idx_1 = robot.VelocityDoFMap[static_cast<int>(vNum) + 10];
