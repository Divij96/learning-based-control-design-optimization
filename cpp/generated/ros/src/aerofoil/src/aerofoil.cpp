...
void aerofoil::step()
{
   ...
   */
  aerofoil_B.VectorConcatenate[0] = aerofoil_X.Integrator_CSTATE[0];

  /* SignalConversion generated from: '<S1>/Vector Concatenate' incorporates:
   *  Concatenate: '<S1>/Vector Concatenate'
   *  Integrator: '<S1>/Integrator'
   */
  aerofoil_B.VectorConcatenate[1] = aerofoil_X.Integrator_CSTATE[1];

  /* SignalConversion generated from: '<S1>/Vector Concatenate' incorporates:
   *  Concatenate: '<S1>/Vector Concatenate'
   *  Inport: '<Root>/In4'
   */
  aerofoil_B.VectorConcatenate[2] = aerofoil_U.delta;

  /* SignalConversion generated from: '<S1>/Vector Concatenate' incorporates:
   *  Concatenate: '<S1>/Vector Concatenate'
   *  Integrator: '<S1>/Integrator'
   */
  aerofoil_B.VectorConcatenate[3] = aerofoil_X.Integrator_CSTATE[3];
  aerofoil_B.VectorConcatenate[4] = aerofoil_X.Integrator_CSTATE[4];
  if (rtmIsMajorTimeStep((&aerofoil_M))) {
     ...
     */
    rtb_TSamp = aerofoil_U.delta * 1000.0;

    /* Sum: '<S2>/Diff' incorporates:
     *  Concatenate: '<S1>/Vector Concatenate'
     *  UnitDelay: '<S2>/UD'
     */
    aerofoil_B.VectorConcatenate[5] = rtb_TSamp - aerofoil_DW.UD_DSTATE;
  }
   ...
   */
  for (i = 0; i < 6; i++) {
    aerofoil_B.VectorConcatenate[i + 6] = aerofoil_X.Integrator_CSTATE[i + 6];
  }
   ...
   *  Integrator: '<S1>/Integrator'
   */
  aerofoil_Y.pitch = aerofoil_X.Integrator_CSTATE[0];

  /* Outport: '<Root>/plunge' incorporates:
   *  Integrator: '<S1>/Integrator'
   */
  aerofoil_Y.plunge = aerofoil_X.Integrator_CSTATE[1];

  /* Outport: '<Root>/flap' incorporates:
   *  Integrator: '<S1>/Integrator'
   */
  aerofoil_Y.flap = aerofoil_X.Integrator_CSTATE[2];
  ...
  for (i = 0; i < 12; i++) {
    ...
    for (i_0 = 0; i_0 < 12; i_0++) {
      tmp_0 += aerofoil_U.MatrixA[12 * i_0 + i] *
        aerofoil_B.VectorConcatenate[i_0];
    }

    aerofoil_B.Add[i] = aerofoil_U.MatrixB[i] * aerofoil_U.delta + tmp_0;
  ...
  if (rtmIsMajorTimeStep((&aerofoil_M))) {
    ...
    if (rtmIsMajorTimeStep((&aerofoil_M))) {
      ...
      aerofoil_DW.UD_DSTATE = rtb_TSamp;
    }
...
void aerofoil::initialize()
{
  ...
  (&aerofoil_M)->Timing.stepSize0 = 0.001;
