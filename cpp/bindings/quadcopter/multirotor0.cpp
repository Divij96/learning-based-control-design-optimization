 ...
 * C++ source code generated on : Fri Jan  5 18:23:34 2024
 *
...
void multirotor0::step0()              /* Sample time: [0.0s, 0.0s] */
{
   ...
   *  Inport: '<Root>/Att_init'
   */
  phi = multirotor0_U.Att_init[0] / 2.0;
  theta = multirotor0_U.Att_init[1] / 2.0;
  psi = multirotor0_U.Att_init[2] / 2.0;
  cphi = std::cos(phi);
  phi = std::sin(phi);
  ...
  q2 = cphi * theta;
  q1_tmp = phi * ctheta;
  q1 = q1_tmp * cpsi - q2 * spsi;
  q2 = q2 * cpsi + q1_tmp * spsi;
  ...
  multirotor0_B.quat_output[0] = psi;
  multirotor0_B.quat_output[1] = q1;
  multirotor0_B.quat_output[2] = q2;
  multirotor0_B.quat_output[3] = cphi;
   ...
   *  Product: '<S25>/Product'
   */
  spsi = rtb_Divide[0] * rtb_Divide[0];

  /* Product: '<S24>/Product2' incorporates:
   *  Product: '<S25>/Product2'
   */
  psi_tmp = rtb_Divide[1] * rtb_Divide[1];
   ...
   */
  psi_tmp_0 = ((spsi + psi_tmp) - cphi) - phi;
  rtb_VectorConcatenate[0] = psi_tmp_0;

  /* Product: '<S29>/Product' incorporates:
   *  Product: '<S27>/Product'
   */
  q2 = rtb_Divide[1] * rtb_Divide[2];
   ...
   *  Sum: '<S29>/Add'
   */
  rtb_VectorConcatenate[1] = (q2 - ctheta) * 2.0;

  /* Product: '<S31>/Product' incorporates:
   *  Product: '<S28>/Product'
   */
  q1_tmp = rtb_Divide[1] * rtb_Divide[3];
   ...
   *  Sum: '<S31>/Add'
   */
  rtb_VectorConcatenate[2] = (q1_tmp + theta) * 2.0;
   ...
   *  Sum: '<S27>/Add'
   */
  rtb_VectorConcatenate[3] = q2 * 2.0;

  /* Sum: '<S25>/Add' incorporates:
   *  Sum: '<S26>/Add'
   */
  spsi -= psi_tmp;
  rtb_VectorConcatenate[4] = (spsi + cphi) - phi;

  /* Product: '<S32>/Product' incorporates:
   *  Product: '<S30>/Product'
   */
  psi_tmp = rtb_Divide[2] * rtb_Divide[3];
   ...
   *  Sum: '<S32>/Add'
   */
  rtb_VectorConcatenate[5] = (psi_tmp - ctheta) * 2.0;
   ...
   *  Sum: '<S28>/Add'
   */
  rtb_VectorConcatenate[6] = q1_tmp * 2.0;
   ...
   *  Sum: '<S30>/Add'
   */
  rtb_VectorConcatenate[7] = psi_tmp * 2.0;

  /* Sum: '<S26>/Add' incorporates:
   *  Fcn: '<S9>/Fcn3'
   */
  spsi = (spsi - cphi) + phi;
  rtb_VectorConcatenate[8] = spsi;
  ...
  if (rtmIsMajorTimeStep((&multirotor0_M))) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      cpsi = multirotor0_U.prop_height[i];
      q1 = theta / 2.0;
      for (xpageoffset = 0; xpageoffset < 3; xpageoffset++) {
        ...
        COM_propeller[Rz_tmp] = (ctheta * 0.0 + psi * theta) + Rz_0 * cpsi;
        COM_airframe[Rz_tmp] = (ctheta * 0.0 + psi * q1) + Rz_0 * 0.0;
      }
    }

    cpsi = COM_propeller[0];
    q1 = COM_propeller[1];
    psi = COM_propeller[2];
    Rz_0 = COM_propeller[3];
    ...
    for (i = 0; i < 4; i++) {
      ...
      y[3 * i] = ((ctheta * q1 + ctheta * cpsi) + ctheta * psi) + ctheta * Rz_0;
      xpageoffset = 3 * i + 1;
    ...
    psi = d_y[2];
    for (xpageoffset = 0; xpageoffset < 3; xpageoffset++) {
      Rz_0 = multirotor0_U.mass_center *
        multirotor0_U.COM_mass_center[xpageoffset];
      COM_system_inter[3 * xpageoffset] = ((phi + cpsi) + Rz_0) / cphi;
      COM_system_inter[3 * xpageoffset + 1] = ((ctheta + q1) + Rz_0) / cphi;
      COM_system_inter[3 * xpageoffset + 2] = ((theta + psi) + Rz_0) / cphi;
    }
    ...
    for (i = 0; i < 4; i++) {
      ...
      theta = multirotor0_U.arm_length[i];
      phi = theta * theta;
      cpsi = 0.083333333333333329 * ctheta * phi;
      std::memset(&I_propeller_cm[0], 0, 9U * sizeof(real_T));
      I_propeller_cm[0] = cpsi;
      q1 = COM_propeller[i] - COM_system_inter[0];
      d_propeller[0] = q1;
      cphi = q1 * q1;
      I_propeller_cm[4] = cpsi;
      q1 = COM_propeller[i + 4] - COM_system_inter[1];
      d_propeller[1] = q1;
      cphi += q1 * q1;
      I_propeller_cm[8] = 0.0;
      q1 = COM_propeller[i + 8] - COM_system_inter[2];
      d_propeller[2] = q1;
      cphi = (q1 * q1 + cphi) * ctheta;
      ...
      for (xpageoffset = 0; xpageoffset < 3; xpageoffset++) {
        ...
        d_propeller_0[3 * xpageoffset] = d_propeller[0] *
          d_propeller[xpageoffset];
        d_propeller_0[3 * xpageoffset + 1] = d_propeller[1] *
          d_propeller[xpageoffset];
        d_propeller_0[3 * xpageoffset + 2] = q1 * d_propeller[xpageoffset];
      }

      theta = airframe_mass[i];
      cpsi = multirotor0_U.arm_radius[i];
      cpsi *= cpsi;
      q1 = COM_airframe[i] - COM_system_inter[0];
      d_propeller[0] = q1;
      for (xpageoffset = 0; xpageoffset < 9; xpageoffset++) {
        I_propeller_total[xpageoffset] = (cphi * static_cast<real_T>
          (b_I[xpageoffset]) + I_propeller_cm[xpageoffset]) - ctheta *
          d_propeller_0[xpageoffset];
        I_propeller_cm[xpageoffset] = 0.0;
        b_I[xpageoffset] = 0;
      }

      cphi = 0.25 * theta * cpsi + 0.083333333333333329 * theta * phi;
      phi = cphi;
      I_propeller_cm[0] = cphi;
      cphi = q1 * q1;
      I_propeller_cm[4] = phi;
      q1 = COM_airframe[i + 4] - COM_system_inter[1];
      d_propeller[1] = q1;
      cphi += q1 * q1;
      I_propeller_cm[8] = 0.5 * theta * cpsi;
      q1 = COM_airframe[i + 8] - COM_system_inter[2];
      d_propeller[2] = q1;
      cphi = (q1 * q1 + cphi) * theta;
      for (xpageoffset = 0; xpageoffset < 3; xpageoffset++) {
        b_I[xpageoffset + 3 * xpageoffset] = 1;
        d_propeller_0[3 * xpageoffset] = d_propeller[0] *
          d_propeller[xpageoffset];
    ...
    for (xpageoffset = 0; xpageoffset < 3; xpageoffset++) {
      multirotor0_B.COM_system[xpageoffset] = COM_system_inter[xpageoffset];
    }
  ...
  for (ForEach_itr = 0; ForEach_itr < 4; ForEach_itr++) {
     ...
     */
    cpsi = multirotor0_B.MotorMatrix_real[ForEach_itr + 44];
     ...
     *  RelationalOperator: '<S67>/LowerRelop1'
     */
    if (multirotor0_X.CoreSubsys[ForEach_itr].Integrator_CSTATE > cpsi) {
    ...
    if (rtmIsMajorTimeStep((&multirotor0_M))) {
       ...
       */
      multirotor0_B.CoreSubsys[ForEach_itr].VectorConcatenate[1] = Rz_0 * phi -
        theta * q1;

      /* Product: '<S111>/Product1' incorporates:
       *  Product: '<S112>/Product'
       */
      COM_propeller_0 = theta * cpsi;
       ...
       */
      multirotor0_B.CoreSubsys[ForEach_itr].VectorConcatenate[4] = Rz_0 * q1 +
        theta * phi;
     ...
     *  Sum: '<S60>/Sum'
     */
    q1 = (multirotor0_X.omega_CSTATE[1] * multirotor0_B.CoreSubsys[ForEach_itr].
          VectorfromrealCoGtopropellerBod[2] -
          multirotor0_B.CoreSubsys[ForEach_itr].VectorfromrealCoGtopropellerBod
          [1] * multirotor0_X.omega_CSTATE[2]) + rtb_TrueairspeedBodyaxes[0];
    ...
    for (xpageoffset = 0; xpageoffset < 3; xpageoffset++) {
      ...
      rtb_Sum_a[xpageoffset] = (multirotor0_B.CoreSubsys[ForEach_itr].
        VectorConcatenate[xpageoffset + 3] * psi +
        multirotor0_B.CoreSubsys[ForEach_itr].VectorConcatenate[xpageoffset] *
        q1) + multirotor0_B.CoreSubsys[ForEach_itr]
        .VectorConcatenate[xpageoffset + 6] * cpsi;
    }
    ...
    if (rtmIsMajorTimeStep((&multirotor0_M))) {
       ...
       *  Product: '<S98>/Divide'
       */
      cpsi = multirotor0_B.CoreSubsys[ForEach_itr].Climbspeedv_c / 4.0;
      ...
      switch (rtAction) {
       ...
       case 1:
         ...
         *  Sum: '<S98>/Add'
         */
        multirotor0_B.CoreSubsys[ForEach_itr].Merge = ((((-1.125 * cpsi + 1.0) +
          theta) + -1.718 * ctheta) + ctheta * cpsi * -0.655) * 4.0;
    ...
    ctheta = (multirotor0_B.MotorMatrix_real[ForEach_itr + 24] * cphi +
              multirotor0_B.MotorMatrix_real[ForEach_itr + 28] * Rz_0) * phi;
    ...
    if (rtAction == 0) {
       ...
       */
      COM_propeller_0 = -COM_propeller_0 * 0.23 * cpsi;
      ...
      for (xpageoffset = 0; xpageoffset < 3; xpageoffset++) {
         ...
         */
        multirotor0_B.CoreSubsys[ForEach_itr]
          .NewtiltedthrustdirectionBodyaxe[xpageoffset] = (COM_propeller_2 * psi
          + COM_propeller_1 * q1) + COM_propeller_3 * theta;
     ...
     *  Merge: '<S76>/Merge'
     */
    d_propeller[0] = ctheta * multirotor0_B.CoreSubsys[ForEach_itr].
      NewtiltedthrustdirectionBodyaxe[0];
    d_propeller[1] = ctheta * multirotor0_B.CoreSubsys[ForEach_itr].
      NewtiltedthrustdirectionBodyaxe[1];
    d_propeller[2] = ctheta * multirotor0_B.CoreSubsys[ForEach_itr].
     ...
     */
    cpsi = multirotor0_B.MotorMatrix_real[ForEach_itr + 12];
    ...
    theta = (multirotor0_B.MotorMatrix_real[ForEach_itr + 32] * cphi +
             multirotor0_B.MotorMatrix_real[ForEach_itr + 36] * Rz_0) * cpsi;

    /* Product: '<S78>/Product5' */
    cpsi *= multirotor0_B.CoreSubsys[ForEach_itr].Gain1;
    ...
    rtb_ImpAsg_InsertedFor_Motor_mo[i] = ((multirotor0_B.CoreSubsys[ForEach_itr]
      ...
      multirotor0_X.omega_CSTATE[0] * multirotor0_B.CoreSubsys[ForEach_itr].
      NewtiltedthrustdirectionBodyaxe[2]) * cpsi * ctheta + (theta *
      multirotor0_B.CoreSubsys[ForEach_itr].Product9[1] * phi +
      multirotor0_B.CoreSubsys[ForEach_itr].Momentinthemotorhubduetobending[1]))
      + (d_propeller[0] * multirotor0_B.CoreSubsys[ForEach_itr].
         VectorfromrealCoGtopropellerBod[2] -
         multirotor0_B.CoreSubsys[ForEach_itr].VectorfromrealCoGtopropellerBod[0]
         * d_propeller[2]);
     ...
     *  Product: '<S73>/Product9'
     */
    rtb_ImpAsg_InsertedFor_Motor_fo[i] = d_propeller[1];
    ...
    rtb_ImpAsg_InsertedFor_Motor_mo[i] = ((multirotor0_X.omega_CSTATE[0] *
      multirotor0_B.CoreSubsys[ForEach_itr].NewtiltedthrustdirectionBodyaxe[1] -
      multirotor0_B.CoreSubsys[ForEach_itr].NewtiltedthrustdirectionBodyaxe[0] *
      multirotor0_X.omega_CSTATE[1]) * cpsi * ctheta + (theta *
      multirotor0_B.CoreSubsys[ForEach_itr].Product9[2] * phi +
      multirotor0_B.CoreSubsys[ForEach_itr].Momentinthemotorhubduetobending[2]))
      + (multirotor0_B.CoreSubsys[ForEach_itr].VectorfromrealCoGtopropellerBod[0]
         * d_propeller[1] - d_propeller[0] *
         multirotor0_B.CoreSubsys[ForEach_itr].VectorfromrealCoGtopropellerBod[1]);

    /* ForEachSliceAssignment generated from: '<S57>/Motor_force' incorporates:
     *  Product: '<S73>/Product9'
     */
    rtb_ImpAsg_InsertedFor_Motor_fo[i] = d_propeller[2];
  }
  ...
  if (rtAction == 0) {
     ...
     *  Sum: '<S41>/Sum1'
     */
    cpsi = (rtb_TrueairspeedBodyaxes_b[0] * rtb_TrueairspeedBodyaxes_b[0] +
     ...
     *  Sqrt: '<S52>/Sqrt'
     */
    phi = std::abs(cpsi * 0.6125 * 0.4 * std::sqrt(phi * phi + ctheta));
  ...
  d_propeller[0] = multirotor0_X.V_b_CSTATE[1] * multirotor0_X.omega_CSTATE[2];
  d_propeller[1] = multirotor0_X.omega_CSTATE[0] * multirotor0_X.V_b_CSTATE[2];
  d_propeller[2] = multirotor0_X.V_b_CSTATE[0] * multirotor0_X.omega_CSTATE[1];
  ...
  cpsi = multirotor0_X.omega_CSTATE[0];
  q1 = multirotor0_X.omega_CSTATE[2];
  for (xpageoffset = 0; xpageoffset < 3; xpageoffset++) {
    ...
    multirotor0_B.Sum1[xpageoffset] = (((((rtb_VectorConcatenate[xpageoffset + 3]
      * cphi + rtb_VectorConcatenate[xpageoffset] * phi) +
      ...
      (rtb_TrueairspeedBodyaxes[xpageoffset] - d_propeller[xpageoffset]);
    ...
    rtb_TrueairspeedBodyaxes_b[xpageoffset] =
      (multirotor0_B.inertial_matrix[xpageoffset + 3] * theta +
       multirotor0_B.inertial_matrix[xpageoffset] * cpsi) +
      multirotor0_B.inertial_matrix[xpageoffset + 6] * q1;
  }
  ...
  rtb_TrueairspeedBodyaxes[2] = multirotor0_X.omega_CSTATE[0] *
    rtb_TrueairspeedBodyaxes_b[1];
  d_propeller[0] = rtb_TrueairspeedBodyaxes_b[1] * multirotor0_X.omega_CSTATE[2];
  d_propeller[1] = multirotor0_X.omega_CSTATE[0] * rtb_TrueairspeedBodyaxes_b[2];
  d_propeller[2] = rtb_TrueairspeedBodyaxes_b[0] * multirotor0_X.omega_CSTATE[1];
  for (xpageoffset = 0; xpageoffset < 3; xpageoffset++) {
    rtb_TrueairspeedBodyaxes_b[xpageoffset] =
      rtb_TrueairspeedBodyaxes[xpageoffset] - d_propeller[xpageoffset];
   ...
   *  Trigonometry: '<S9>/Trigonometric Function'
   */
  multirotor0_Y.Euler[1] = -std::asin(q1_tmp);

  /* Trigonometry: '<S9>/Trigonometric Function1' incorporates:
   *  Fcn: '<S9>/Fcn'
   *  Outport: '<Root>/Euler'
   */
  multirotor0_Y.Euler[0] = rt_atan2d_snf(psi_tmp * 2.0, spsi);

  /* Trigonometry: '<S9>/Trigonometric Function2' incorporates:
   *  Fcn: '<S9>/Fcn2'
   *  Outport: '<Root>/Euler'
   */
  multirotor0_Y.Euler[2] = rt_atan2d_snf(q2 * 2.0, psi_tmp_0);
  for (i = 0; i < 3; i++) {
     ...
     *  Integrator: '<S2>/omega'
     */
    cpsi += rtb_Divide[i + 1] * multirotor0_X.omega_CSTATE[i];
   ...
   *  Product: '<S17>/Product'
   */
  multirotor0_B.TmpSignalConversionAtQIntegrato[0] = -0.5 * cpsi;
...
void multirotor0::initialize()
{
...

  {
    ...
    multirotor0_X.V_b_CSTATE[0] = Model_Init.vel_init[0];
    multirotor0_X.V_b_CSTATE[1] = Model_Init.vel_init[1];
    multirotor0_X.V_b_CSTATE[2] = Model_Init.vel_init[2];
    ...
    multirotor0_X.omega_CSTATE[0] = Model_Init.omega_init[0];

    /* InitializeConditions for Integrator: '<S2>/X_i' */
    multirotor0_X.X_i_CSTATE[0] = Model_Init.pos_init[0];
    ...
    /* InitializeConditions for Integrator: '<S2>/omega' */
    multirotor0_X.omega_CSTATE[1] = Model_Init.omega_init[1];

    /* InitializeConditions for Integrator: '<S2>/X_i' */
    multirotor0_X.X_i_CSTATE[1] = Model_Init.pos_init[1];
    ...
    /* InitializeConditions for Integrator: '<S2>/omega' */
    multirotor0_X.omega_CSTATE[2] = Model_Init.omega_init[2];

    /* InitializeConditions for Integrator: '<S2>/X_i' */
    multirotor0_X.X_i_CSTATE[2] = Model_Init.pos_init[2];
