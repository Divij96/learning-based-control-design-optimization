...
def main():
    #training parameters
    save_path = f"{REPO_ROOT}/src/cart_pole/rl/trained_model/schaff_chop/"
    ...
    while True:
        ...
        log_dir = f"{REPO_ROOT}/src/cart_pole/cartpole_tensorboard/TB_{model_name}"

        if LOAD_OLD_MODEL is True:
            new_model = []
            old_model = PPO.load(f"{REPO_ROOT}/src/cart_pole/rl/trained_model/random_dist/new/CartPole_random_design_Tanh_Tsteps_51200000_lr_5e-05_hidden_sizes_256_reward_1.00.20.1_freq_1", env = vec_env)
...
class constant_design(BaseCallback):
    ...
    def _on_rollout_end(self) -> bool:
        ...
        self.average_episode_length = []
        self.average_reward = []
        self.model.save(f"{REPO_ROOT}/src/cart_pole/rl/trained_model/constant_mass/{self.model_name}")
        return True
...
class random_design(BaseCallback):
    ...
    def _on_rollout_end(self) -> bool:
        ...
        self.average_episode_length = []
        self.average_reward = []
        self.model.save(f"{REPO_ROOT}/src/cart_pole/rl/trained_model/random_dist/{self.model_name}")
        return True
