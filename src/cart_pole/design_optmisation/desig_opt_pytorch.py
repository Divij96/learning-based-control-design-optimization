...
class DesignDistribution_log:
    ...
    def update_distribution(self, batch_rewards, batch_samples):
        ...
        for i in range(len(batch_rewards)):
            ...
            log_prob = D.MultivariateNormal(self.mean, self.cov).log_prob(sample)
...
def reward_function(sampled_design):
    reward = torch.norm((sampled_design - torch.tensor([10.0, 20.0, 30, 40], dtype=torch.float32)))
    return reward.item()

initial_mean = 50 * np.random.rand(4).astype(np.float32)
initial_cov = np.eye(4, dtype=np.float32) * 100 + 100
