...
def main():
    ...
    original_xml_path = f"{REPO_ROOT}/src/half_cheetah/assets/half_cheetah.xml"
    destination_folder = f"{REPO_ROOT}/src/half_cheetah/assets/"
    ...
    while True:
        ...
        log_dir = f"{REPO_ROOT}/src/half_cheetah/half_cheetah_tensorboard/TB_{model_name}"

        if LOAD_OLD_MODEL is True:
            new_model = []
            old_model = PPO.load(f"{REPO_ROOT}/src/half_cheetah/rl/trained_model/random_design/Half_cheetah_dynamics_constant_random_design_start_upd_25lr_mean0.01_std_0.01_sdeFalse_Tanh_Tsteps_25600000_lr_0.0001_hidden_sizes_256_FORreward_1.0_CTRLcost_1.0_var_design", env = vec_env)
        ...
        new_model.save(f"{REPO_ROOT}/src/half_cheetah/rl/trained_model/random_design/{model_name}")
        print("Model saved")
        LOAD_OLD_MODEL = True
        vec_env.close()
        break
...
class random_design(BaseCallback):
    def __init__(self, model_name = f"matfile", model = None, n_steps_train=512 * 10, n_envs_train=8, verbose=0):
        ...
        self.mujoco_file_folder = f"{REPO_ROOT}/src/half_cheetah/assets/"
    ...
    def _on_rollout_end(self) -> bool:
        self.model.save(f"{REPO_ROOT}/src/half_cheetah/rl/trained_model/random_design/{self.model_name}")
        ...
        if self.state == "gauss":
            ...
            for i in range(self.n_envs_train//self.batch_size_gauss):
                ...
                if self.design_rewards_avg[i] > self.best_design_reward_gauss:
                    ...
                    self.model.save(f"{REPO_ROOT}/src/half_cheetah/rl/trained_model/bestDesign_{self.model_name}")
            ...
            self.model.save(f"{REPO_ROOT}/src/half_cheetah/rl/trained_model/{self.model_name}")
