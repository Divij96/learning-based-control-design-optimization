...
class Robot_Chain_Env(utils.EzPickle, gym.Env):
    ...
    def __init__(self, REWARD = np.array([0.7, 0.0, 0.3]), env_id = 0, arm_length = [1.0, 1.0], num_links=4 ,call_back = f"random_design"):
        ...
        self.max_torque = 5.0 * np.ones(self.num_links, dtype=np.double)
        self.min_torque = -5.0 * np.ones(self.num_links, dtype=np.double)
        ...
        self.collision_pattern_1 = self.collision_geometry(0.5, start_x=-1, start_y=1.5)
        self.collision_pattern_2 = self.collision_geometry(0.5, start_x=-1, start_y=3.0)
    ...
    def step(self, action):
        ...
        for i, length in enumerate(self.arm_length):
            ...
            for pattern in [self.collision_pattern_1, self.collision_pattern_2]:
        ...
        observation = np.concatenate([obs_joint_torque, obs_pos_vec[:2],
                                      obs_vel_tcp[:2], obs_target_dist ,
                                      obs_joint_pos, obs_joint_vel,
                                      obs_link_pos_task_space, obs_arm_length])
        ...
        info = {'joint_pos': self.curr_joint_pos, 'arm_length': self.arm_length,
                ...
                'collision_pattern_2': self.collision_pattern_2}
    ...
    def reset(self, seed=None):
        ...
        if self.call_back == "random_design":
            ...
            # self.init_joint_pos = random.uniform(self.min_joint_pos, self.max_joint_pos)
            self.init_joint_pos = np.zeros(self.num_links, dtype=np.double)

        self.target_pos_tcp = np.array([0.0, 2.5, 0.0], dtype=np.double)
