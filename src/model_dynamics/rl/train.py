...
class CustomNetwork(nn.Module):
    ...
    def __init__(self, observation_space, net_arch, activation_fn, design_obs=4, trajectory_obs=15):
...
def main():
    ...
    arm_length_limits = np.array([0.1, 2.0])
    ...
    while True:
        ...
        env_configs = [{'REWARD': REWARD, 'env_id': i , 'n_steps_train': n_steps_train, 'arm_length_limits' : arm_length_limits, 'callback': CALL_BACK_FUNC} for i in range(n_envs_train)]
        ...
        model_name = f"QuadcopterCircle_all_heboBatch_10_envs_train_{n_envs_train}_batch_size_train_{batch_size_train}_{CALL_BACK_FUNC}_start_upd_{n_update_dist}lr_mean{lr_std_schaff}_std_{lr_std_schaff}_sde{use_sde}_Tanh_Tsteps_{total_timesteps_train}_lr_{learning_rate_train}_hidden_sizes_{hidden_sizes_train}_POSreward_{REWARD[0]}_VELreward_{REWARD[1]}_omega_pen_{REWARD[2]}_var_design"
        log_dir = f"{REPO_ROOT}/src/model_dynamics/Quadcopter_tensorboard/TB_{model_name}"

        if LOAD_OLD_MODEL is True:
            new_model = []
            old_model = PPO.load(f"{REPO_ROOT}/src/model_dynamics/rl/trained_model/random_design/QuadcopterCircle_all_heboBatch_10_envs_train_20_batch_size_train_256_random_design_start_upd_25lr_mean0.01_std_0.01_sdeFalse_Tanh_Tsteps_4096000_lr_0.0001_hidden_sizes_256_POSreward_0.0_VELreward_1.0_omega_pen_0.2_var_design", env = vec_env)
        ...
        new_model.save(f"{REPO_ROOT}/src/model_dynamics/rl/trained_model/random_design/{model_name}")
        print("Model saved")
        LOAD_OLD_MODEL = True
        vec_env.close()
        break
...
class random_design(BaseCallback):
    ...
    def _on_rollout_start(self) -> bool:
        ...
        for i in range(self.n_envs_train):
            ...
            self.training_env.env_method('set_steps', 0, indices=[i])
    ...
    def _on_rollout_end(self) -> bool:
        ...
        self.average_reward = []
        self.design_iteration = [0 for _ in range(self.n_envs_train)]
        self.model.save(f"{REPO_ROOT}/src/model_dynamics/rl/trained_model/random_design/{self.model_name}")
        return True
...
class Schaff_callback(BaseCallback):
    def __init__(self,model_name = f"matfile" ,n_update_dist = 1000,n_steps_train=512 * 10, n_envs_train=8,lr_mean_schaff=0.01,lr_std_schaff=0.01, n_number_dist = 8, verbose=0):
        ...
        for _ in range(self.num_distributions):
            self.initial_mean = np.array([min_val + (max_val - min_val) * random.rand()
    ...
    def _on_step(self) -> bool:
        ...
        if self.num_timesteps > self.steps_update_distribution:
            ...
            if self.num_timesteps % self.save_recorded_data == 0:
                ...
                self.model.save(f"{REPO_ROOT}/src/model_dynamics/rl/trained_model/{self.model_name}_{self.mat_iter}")
...
class Hebo_callback(BaseCallback):
    ...
    def random_point_on_circle(self, r, theta):
        x = r * np.cos(theta)
        y = r * np.sin(theta)
    ...
    def on_training_start(self, locals_: Dict[str, Any], globals_: Dict[str, Any]) -> bool:
        ...
        theta = np.linspace(0, 2*np.pi, self.batch_size_opt)
        att = np.linspace(0, 2*np.pi, self.batch_size_opt)

        for i in range(self.batch_size_opt):
            self.init_pos.append(self.random_point_on_circle(self.major_radius, theta[i]))
            self.init_Att.append(np.array([att[i], att[i], att[i]]))
    ...
    def _on_rollout_start(self) -> bool:
        ...
        for i in range(self.n_envs_train//self.batch_size_opt):
            for j in range(self.batch_size_opt):
                new_arm_length = [self.rec.values[i][0], self.rec.values[i][1], self.rec.values[i][2], self.rec.values[i][3]]
                ...
                self.training_env.env_method('set_init_pos', self.init_pos[j], indices=[i*self.batch_size_opt + j])
                self.training_env.env_method('set_int_Att', self.init_Att[j], indices=[i*self.batch_size_opt + j])
                ...
                init_pos = self.training_env.env_method('get_init_pos', indices=[i*self.batch_size_opt + j])[0]
                print(f"env: {i*self.batch_size_opt + j:<1.2f}, real id:{dist_env_id}, pole length: {new_arm_length}, init pos: {init_pos}")
    ...
    def _on_rollout_end(self) -> bool:
        ...
        if self.num_timesteps % self.save_recorded_data == 0:
            ...
            file_path = f"{REPO_ROOT}/MultirotorSim_Vervoorst/{self.mat_file_name}.mat"
            savemat(file_path, output_data)
