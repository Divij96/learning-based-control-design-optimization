...
def main():
    ...
    while True:
        ...
        log_dir = f"{REPO_ROOT}/src/forward_dynamics_robot_arm/PlanarRobot_tensorboard/TB_{model_name}"

        if LOAD_OLD_MODEL is True:
            new_model = []
            old_model = PPO.load(f"{REPO_ROOT}/src/forward_dynamics_robot_arm/rl/trained_model/random_design/ScaraRobot_dynamics_random_design_start_upd_25lr_mean0.01_std_0.01_sdeFalse_Tanh_Tsteps_20480000_lr_0.0001_hidden_sizes_256_POSreward_1.0_VELreward_0.0_omega_pen_0.0_var_design", env = vec_env)
        ...
        new_model.save(f"{REPO_ROOT}/src/forward_dynamics_robot_arm/rl/trained_model/random_design/{model_name}")
        print("Model saved")
        LOAD_OLD_MODEL = True
        vec_env.close()
        break
...
class constant_design(BaseCallback):
    ...
    def _on_rollout_end(self) -> bool:
        self.model.save(f"{REPO_ROOT}/src/forward_dynamics_robot_arm/rl/trained_model/random_design/{self.model_name}")
    ...
    def _on_step(self) -> bool:
        ...
        if self.num_timesteps > self.steps_update_distribution:
            ...
            if self.num_timesteps % self.save_recorded_data == 0:
                ...
                self.mat_iter += 1
                file_path = f"{REPO_ROOT}/MultirotorSim_Vervoorst/{self.mat_file_name}_{self.mat_iter}.mat"
                savemat(file_path, output_data)
                self.model.save(f"{REPO_ROOT}/src/forward_dynamics_robot_arm/rl/trained_model/{self.model_name}_{self.mat_iter}")
...
class Hebo_callback(BaseCallback):
    ...
    def _on_rollout_end(self) -> bool:
        ...
        if self.num_timesteps % self.save_recorded_data == 0:
            ...
            file_path = f"{REPO_ROOT}/robot/{self.mat_file_name}.mat"
            savemat(file_path, output_data)
...
class Hebo_Gauss_callback(BaseCallback):
    ...
    def _on_rollout_end(self) -> bool:
        ...
        if self.state == "hebo":
            ...
            file_path = f"{REPO_ROOT}/robot/{self.mat_file_name}_{self.hebo_init_ctr}.mat"
            savemat(file_path, output_data)
        ...
        if self.state == "gauss":
            ...
            if np.mean(self.design_rewards_avg) >= self.logger_reward_prev:
                self.model.save(f"{REPO_ROOT}/src/forward_dynamics_robot_arm/rl/trained_model/{self.model_name}_{self.gauss_init_ctr}")
