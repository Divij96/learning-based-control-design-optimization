...
def main():
    ...
    original_xml_path = f"{REPO_ROOT}/src/ant/assets/ant.xml"
    destination_folder = f"{REPO_ROOT}/src/ant/assets/"
    ...
    while True:
        ...
        log_dir = f"{REPO_ROOT}/src/ant/ant_tensorboard/TB_{model_name}"

        if LOAD_OLD_MODEL is True:
            new_model = []
            old_model = PPO.load(f"{REPO_ROOT}/src/ant/rl/trained_model/random_design/ant_FullGeom_random_design_ent_0.0_lr_0.0001_hid_size_256_FORreward_1.0_CTRLcost_0.0", env = vec_env)
        ...
        new_model.save(f"{REPO_ROOT}/src/ant/rl/trained_model/random_design/{model_name}")
        print("Model saved")
        LOAD_OLD_MODEL = True
        vec_env.close()
        break
...
class random_design(BaseCallback):
    def __init__(self, model_name = f"matfile", model = None, n_steps_train=512 * 10, n_envs_train=8, verbose=0):
        ...
        self.mujoco_file_folder = f"{REPO_ROOT}/src/ant/assets/"
    ...
    def _on_rollout_end(self) -> bool:
        self.model.save(f"{REPO_ROOT}/src/ant/rl/trained_model/random_design/{self.model_name}")
...
class Hebo_Gauss_callback(BaseCallback):
    ...
    def _on_rollout_end(self) -> bool:
        ...
        if self.state == "hebo":
            ...
            for i in range(self.n_envs_train//self.batch_size_hebo):
                ...
                if self.design_rewards_avg[i] > self.mat_best_reward_policy:
                    ...
                    self.model.save(f"{REPO_ROOT}/src/ant/rl/trained_model/bestHeboDesign_{self.model_name}")
        ...
        if self.state == "gauss":
            ...
            for i in range(self.n_envs_train//self.batch_size_gauss):
                ...
                if self.design_rewards_avg[i] > self.best_design_reward_gauss:
                    ...
                    self.model.save(f"{REPO_ROOT}/src/ant/rl/trained_model/bestGaussDesign_{self.model_name}")
            ...
            self.model.save(f"{REPO_ROOT}/src/ant/rl/trained_model/{self.model_name}")
        ...
        file_path = f"{REPO_ROOT}/src/ant/rl/trained_model/{self.mat_file_name}.mat"
