num_links = 6;
arm_length = 2 * ones(num_links,1);
init_pos = 0.1 * ones(num_links,1);
config = 0 * ones(num_links,1);
vel = 0 * ones(num_links,1);
tq = 0 * ones(1,num_links);
rho = 2000;
dt = 0.001;
radius = 1.0;
damping_factor = 1;
joint_angles = 0.0 * ones(num_links,1);

joint_angles = [0 pi/2 0 pi/2 0 0];
...
joint_axes = [0 0 1;
    ...
    [acc, linkPositionsArray] = forward_dynamics_robot_6arm(rho, radius, arm_length,joint_axes,joint_angles,config,vel,tq)
...
elseif num_links == 4
    joint_axes = [1 0 0;
                  ...
                  0 0 1;
                  0 0 1];
    acc_temp = forward_dynamics_robot_4arm(rho, radius, arm_length,joint_axes,joint_angles,config,vel,tq)
end
